...
*Last updated: 2026-09-16 — Token-limit support (ADR-162): `.p-trunc`, the card under a cut-off answer (fork-anchor grammar in the warning colour, `.p-trunc-btn` actions like `.p-del-compare`); `.p-param-advice` under the modal's token field (warning-coloured text + accent-outline button, hidden when the rule is silent); `.p-model-pop-profile`, the mono speed · depth · cost line in popover rows. Earlier: Conversation settings modal: every help text now states the effect of raising or lowering the value (see the modal section). Earlier the same day: ADR-155 applied to the rest of the stylesheet (ADR-161). Every `:hover` rule that changes a **background fill** (22 of them: header buttons, model badge and popover rows, nav items and tree rows, tool buttons, selection and Send-menu items, the delete bar, term/person/merge marks, the summary-card header, the inspector's summarize button, code-block buttons, the `#` trigger) now sits under `@media (hover: hover)`, so a tap on iOS no longer leaves a hover fill stuck on the last-touched control. Colour-only and underline hovers stay unwrapped — a stuck text colour is harmless, a stuck fill reads as a second selection. `.p-tool-btn` transitions colour only: its `.is-active` accent fill is state (research / vault toggles) and must be true at the tap. The model popover's shadow is the theme's `--shadow-l`, not a hardcoded `rgba`.*

*Previously: 2026-09-16 — comparison card (ADR-160). The last-bubble action bar gains a third control, `.p-del-compare` (`⇄ Compare`, accent outline — an action, beside the error-outlined delete and the plain cancel). The card `.p-compare` sits where the assistant turn would be and borrows the fork anchor's grammar: 2px accent left rule, `git-compare` icon + mono caps `COMPARISON` label in `--text-muted` 600. Tabs (`.p-compare-tab`, mono `--font-smaller`, one per model) under a hairline; the active tab is `--text-normal` 600 with a 2px accent bottom rule and **no transition** (state, ADR-155). Body is `.p-ai-body`; meta line is the 9px mono turn-label rhythm (`MODEL · 04:39 · ↑n ↓m`). Actions: `Keep this answer` (accent fill, `--p-on-accent` with `-webkit-text-fill-color`, ADR-154), `+ Model` (accent outline), `Discard comparison` (faint). All hover rules under `@media (hover: hover)`.*

*Previously: 2026-09-15 — marks nest (ADR-157). A glossary term is now marked inside a favorite, fork origin or merge link instead of being skipped there, so a highlighted passage keeps its dotted term underlines: the highlighter fill and the term's underline simply stack. Tap precedence is no longer by type — **the innermost mark under the finger wins**, because the outer mark stays tappable along the rest of its span while the inner one has nowhere else to be tapped.*

*Previously, 2026-09-15 — the glossary card sits beside its word (ADR-156). `GlossaryController` now inserts the anchor with `markEl.after(anchor)`, the same call `ForkController` and `MergeController` use, instead of after the mark's enclosing block. The paragraph splits at the term while the card is open — the same cost fork already pays — and in exchange the definition is beside the word rather than at the end of a paragraph that may be long. **Placement is part of the component**: the four cards differ only in the stroke of their left rule.*
...
**Help texts state the effect, not the mechanism.** Every row's description says what changes when the value goes up or down and what does not change, in the order a user decides: what it does → low → high → what stays the same → what "default" means. Temperature: focus vs. variety, no effect on length or cost. Effort: thinking time vs. speed and cost, and that thinking counts against the token limit. Max tokens: a length cap that truncates mid-sentence when hit, with a rough page equivalent, that only written tokens are paid for, and that reasoning models spend the same budget on thinking. Language: applies to generated text too, and never translates what is already written. Theme: a `[[link]]` in each term note, so backlinks; empty follows renames, a value pins. Provider and model carry descriptions as well (`providerDesc`, `modelDesc`): a switch applies from the next message on, and switching provider resets the model. "Overrides the default" alone was the previous text on every row and told the user nothing they could act on. The `paramUnsupportedSuffix` is still appended, unchanged, when a model does not take the parameter.

**Advice under the token field (ADR-162).** `.p-param-advice` sits directly under the max-tokens row and is `display: none` unless `maxTokensAdvice` returns something: warning-coloured text naming the effective and recommended values, and one accent-outline button. The button's label is the action, not "fix": `Use the default (16384)` when clearing the conversation's override is enough (the value then follows the model again), `Set 16384 for this conversation` when the global setting is the low value. It updates live with the provider and model dropdowns and fires on open when the conflict already exists. The same rule drives the warning triangle beside Send and the card under a cut-off answer — never restate the threshold in CSS or a second function.

**Cut-off answer card (`.p-trunc`).** Under an assistant answer whose stream stopped at the token cap. The fork anchor's grammar — 2px left rule, 11px icon, mono 9px caps label `CUT OFF`, 11px meta — in `--text-warning` rather than accent, because it reports a fault, not a branch point. The meta names the limit it hit and, on a reasoning model, that the thinking counts against it. On the last answer only, a wrapping `.p-trunc-actions` row: `→ Continue` and `↑ Retry with N` as `.p-trunc-btn` (accent outline, like `.p-del-compare`), `⇄ Compare` as `.p-trunc-btn--quiet` (border-colour border, faint text). No fill, no transition on state; hover under `@media (hover: hover)`; 32px minimum height on coarse pointers.

**Model rows (popover and picker).** Every row's hover/armed area carries, after the "good for" line, `.p-model-pop-profile`: `Speed ●●○ · Depth ●●● · Cost ●●●` in the mono face so the dots align down the list; a reasoning row adds one plain line saying it needs a bigger token limit. `ModelSuggest` appends the same profile to its item text.
