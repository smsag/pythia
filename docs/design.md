...
*Last updated: 2026-08-27 — favorite/fork highlights adopt smsag.de's "highlighter marker" look (ADR-090): asymmetric `1em 0 1em 0` corners, a diagonal `linear-gradient(-100deg)` ink sweep, and a theme-adaptive `--background-primary` text-shadow. Colors unchanged (favorite yellow / fork accent); always-visible, not hover-gated.*
