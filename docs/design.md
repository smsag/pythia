...
*Last updated: 2026-09-15 — the panel fills its leaf at rest again (ADR-132). A dead strip of ~67px sat below the composer because the view shrank its own content pane on every viewport event, keyboard or not, and `overflow: hidden` then clipped the input area's safe-area padding. The panel is now left untouched unless a soft keyboard is actually open, and keyboard avoidance lifts content with `padding-bottom` so the panel keeps painting its full leaf. **Rule: never set an explicit `height` on the view's content pane.** It is `overflow: hidden`, so any height smaller than the content silently crops the bottom of the input area and uncovers the background behind the panel. Use padding to move content instead.*
...
- `.pythia-view .p-scroll-frame > table` gets `max-width: none`, so the table may become wider than its frame.
- `.pythia-view .p-scroll-frame > table th, td` get `overflow-wrap: normal`, `word-break: normal`, `hyphens: none` and `min-width: 8ch`.

The `.pythia-view` scope is required, not decorative: Obsidian core and themes style table cells with `word-break: break-all` or `overflow-wrap: anywhere` and load after the plugin, so an unscoped rule loses (ADR-065).
