...
*Last updated: 2026-09-15 — the composer gives 8px back (ADR-148). `.p-input-area` is now `padding: var(--s1) var(--s3) var(--s1)` with `gap: var(--s2)`: the textarea and the toolbar read as one control, and the 8px above the textarea was inherited from a desktop-sized composer that the `border-top` already separates from the transcript. The remaining ~36px band below the panel — a **linear** gradient inset ~11px on both sides, while the panel itself is flush — is painted by an ancestor in Obsidian's mobile drawer, not by anything in `styles.css`; whether it sits inside our panel or below it is what the new `Diagnose panel layout` command answers, because both surfaces are `--background-primary` and a screenshot cannot separate them.*
...
**Box (ADR-146 / ADR-148):** `.p-input-area` is `padding: var(--s1) var(--s3) var(--s1)` with `gap: var(--s2)` between the textarea and the toolbar — 4px top and bottom, 12px sides, 8px between the two rows. No `env(safe-area-inset-bottom)` (hard rule 7). The composer is the panel's floor, so every pixel it does not need is transcript: the 8px above the textarea went because the `border-top` already separates it from the chat, and the 12px gap went because the textarea and the toolbar are one control, not two rows.
...
### Conversation settings modal (temperature, max tokens)
