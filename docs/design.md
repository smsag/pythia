...
*Last updated: 2026-09-10 — locale pass over `locales/de.ts` against this doc. Parity is clean (380/380 keys) and every literal specified here matches; three wording fixes landed: the prompt optimizer's Send-button label `Wird optimiert…` → `Optimiere…` (the pill carries short labels only), the history subtitle `Zweig` → `Abzweigung` (the fork family is Abzweigung / Verzweigen / Verzweigt von everywhere else), and the Favorites wording — empty state `Noch keine Markierungen` → `Noch keine Favoriten` (English `No highlights yet` → `No favorites yet`, same drift) and the selection toolbar's `Entfernen` → `Favorit entfernen`.*
