...
*Last updated: 2026-09-18 — first catalog curation (ADR-179 addendum). `MODEL_CATALOG` hides `gpt-4.1-nano`, `o3-mini`, `o4-mini` and adds `claude-fable-5-1`, `gpt-5.6`, `gpt-5.4-mini`, `gpt-5.4-nano`, `mistral-medium-latest` (profiles, guidance, prices). `scripts/update-models.mjs` gains `upstreamWindow`: `contextWindow` is `limit.input` where upstream gives one, else `limit.context`. `DEFAULT_SETTINGS.defaultOpenAIModel` is `gpt-5.4-mini`. +3 tests (1265 across 84 files).*
