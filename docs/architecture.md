...
*Last updated: 2026-08-28 — ADR-106: conversation search now ranks by lexical TF-IDF over content (title + LLM summary + message bodies) via `services/conversationSearch.ts` + the existing `noteRelevance` scorer. Wired into all three surfaces — the in-panel history view (F10) and quick switcher (F9) in `ui/HistoryController.ts` (empty query → date-grouped listing; active query → flat relevance-ranked list with a match snippet per row) and the command-palette `ConversationSuggestModal`. Chosen over on-device semantic embeddings (transformers.js/MiniLM, per obsidian-similarity) — documented as the Phase-2 seam. 462 tests.*
