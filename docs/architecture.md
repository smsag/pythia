...
*Last updated: 2026-09-15 — summary contract and clamp (ADR-141). `services/BaseProvider.ts` gained a module-level `SUMMARY_RULES` shared by `generateSummary` and `generateSummaryWithTitle`, which previously duplicated their instructions; token caps unchanged. New `ui/clampBody.ts` (`clampSummary(body, mount)`) bounds the fork and merge anchor summaries and is wired into both controllers. New `tests/clampBody.test.ts`; +6 tests (758 across 50 files).*
