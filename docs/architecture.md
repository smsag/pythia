...
| `ui/MergeController.ts` | 232 | Merge-link display (ADR-130) — the inverse of ForkController: the painted `<pythia-merge class="p-merge-link">` marks on passages pointed at another conversation (`repaintMergeLinks` over HighlightPainter), the inline `.p-merge-anchor` those marks open (target name + its conversation summary + `N messages · MODEL · date [· outdated]` + regenerate / unlink / `Open →`), plus `revealMergeLink` and `removeMergeLink`. `Deps`-driven, same shape as ForkController. Creating a merge from a selection lives in SelectionController + `cmdMergeConversation`. Regeneration uses `generateSummary` (never `generateSummaryWithTitle`) so merging never renames the target |
...
| `tests/` | — | Vitest unit tests (660 tests across 42 files, ~4 s) |
...
└── PythiaSidebarView (sidebar.ts)
    ...
    ├── MergeController        — merge-link marks + inline merge anchor (ADR-130)
