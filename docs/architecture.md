...
*Last updated: 2026-09-15 — ADR-135. `ui/keyboardInset.ts` gains `watchViewport(onChange)`, which owns the `visualViewport` listeners and returns a disposer; `sidebar.ts` also subscribes to the workspace's `resize` and `layout-change`, because a leaf opening or closing moves the panel's bottom edge without firing any viewport event and would otherwise leave `--p-bottom-inset` stale. `sidebar.ts` 1,903 → 1,891 lines, ceiling lowered.*
...
| `tests/` | — | Vitest unit tests (694 tests across 46 files, ~5 s) |
