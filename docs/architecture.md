...
| `services/conversationSearch.ts` | 96 | Lexical conversation search (ADR-106): `buildConversationHaystack` (title ×3 + LLM summary + message bodies), `rankConversations` (empty → recency, else TF-IDF score-desc via `noteRelevance`), `bestMatchSnippet` (why-it-matched line for the picker) |
