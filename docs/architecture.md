...
| `ui/keyboardInset.ts` | 66 | Soft-keyboard avoidance arithmetic (ADR-132): `keyboardOverlap(metrics)` returns how much of the panel the keyboard covers, and **0 whenever no keyboard is open** so the panel is never resized at rest; `MIN_KEYBOARD_INSET` (120px) separates a real keyboard from Obsidian chrome and the home indicator. Pure and unit-tested because `visualViewport` and a soft keyboard cannot be reproduced headlessly; `currentKeyboardOverlap(container)` is the DOM-reading shell the view calls |
...
| `tests/` | — | Vitest unit tests (690 tests across 46 files, ~5 s) |
