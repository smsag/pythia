...
| `services/glossary.ts` | 160 | Pure glossary core (ADR-136): `parseGlossary` / `upsertGlossaryEntry` for the `## Term` note format (tolerant, preserves hand edits and the preamble), `normalizeTerm`, and `buildTermMatcher` — ONE alternation over every term, longest-first (JS alternation is first-match, not longest-match) with `\p{L}\p{N}_` lookarounds instead of ASCII-only `\b` |
| `services/GlossaryService.ts` | 175 | Owns the glossary note: reads and caches entries, resolves a term vault-first then via `llmRouter.defineTerm(term, passage)`, writes the result back, de-duplicates in-flight lookups. Read-modify-write against the file, never the cache, so a hand edit between lookups is not clobbered |
