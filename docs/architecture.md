...
*Last updated: 2026-09-18 — a shared label is not a shared number (ADR-176). `RelatedSimilarity` → `SimilarityPreset` (+ `SIMILARITY_PRESETS`, `DEFAULT_SIMILARITY_PRESET`) across `models/embeddingModels.ts`, `models/settings.ts`, `services/persistence.ts`, `services/embedding/relatedConversations.ts` and `ui/embeddingSettings.ts`: the type named the measured related-conversation floors while also typing the unmeasured vault-retrieval preset. The resolvers (`relatedMinScore`, `vaultRetrievalMinScore`) and `relatedFloors` keep their names — that is where the difference lives. Pure rename; no behaviour, no persisted key, no migration.*
