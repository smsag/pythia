...
*Last updated: 2026-09-15 — layout instrumentation (ADR-148). New `ui/layoutReport.ts`: `labelFor` / `readBox` / `ancestorChain` / `probeStrip` / `collectLayoutReport` read the live layout, and the pure `formatLayoutReport` renders it as markdown. `services/ViewManager.ts` gained `diagnoseLayout()`, which writes the report to `<scratchFolder>/Pythia layout report.md` via `NoteWriter` and opens it; `main.ts` registers the `diagnose-layout` command that calls it (the method lives in `ViewManager` rather than `main.ts` to stay under the ADR-097 600-line default). `styles.css` tightens `.p-input-area` (padding-top `--s2`→`--s1`, gap `--s3`→`--s2`). +10 tests (775 across 52 files).*
