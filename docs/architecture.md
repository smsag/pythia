...
*Last updated: 2026-09-18 — the Worker still did not start (ADR-182). `WORKER_ENV_PREFIX` gains a lexical `const process = void 0` beside the `defineProperty`; `FallbackEmbeddingProvider` records per-attempt failure reasons, exposes `backendFailures()`, and passes them to `onBackend(backend, failures)`, logged in `main.ts`. +6 tests (1350 across 89 files).*

*Previously: 2026-09-18 — an index records what it is and whether it finished (ADR-181). `embeddingIndex.ts` format v2: the meta header becomes `{ complete, scope, rows }` with a new `IndexMeta`/`EMPTY_INDEX_META`, validated on read; v1 is refused and rebuilt. `VaultIndexService` gains `isComplete(scope)` / `indexedScope()`, `sync(..., scope)`, and writes `complete: false` on every mid-build flush. `VaultRagService` gains `scopeSignature()` (folders · skip · cap · model), gates `refresh()` and the UI-thread short-circuit on completeness rather than `size() > 0`, buffers mid-build edits in `deferredChanges` and replays them via `flushDeferredChanges()`, and filters retrieved paths through `inScopeNow()` / `optedOutPath()`. `buildAttachedNotesContent` returns `manualTokens`, which is what the size warning reads. +27 tests (1344 across 89 files).*

*Previously: 2026-09-18 — an auto-retrieved note is not an attached one (ADR-180). `LLMRouter` passes the RAG-added paths as `autoNotes` through `LLMProvider.streamMessage` → `BaseProvider.resolveUserContent` → `buildAttachedNotesContent`, which excerpts them to the new `AUTO_NOTE_BUDGET_CHARS`; the missing/oversized warnings go manual-only. `services/embedding/vaultRetrieval.ts` gains pure `retrievalQuery` (message + 200 chars of the preceding answer) and `isIndexingOptedOut` (`pythia: false`), and LOSES `rankByQuery` (dead, duplicated `VaultIndexService.query`). `VaultRagService` reads the opt-out through `optedOut()` (fail-open) on both the full scan and the watcher path, and sorts by `mtime` before the cap. `ui/embeddingSettings.ts` exposes `vaultContextMaxNotes` (1–20). +36 tests (1317 across 88 files).*

*Previously: 2026-09-18 — embedding off the UI thread, and a resumable vault build (ADR-179). `services/embedding/host/embeddingBundle.ts` gains `WORKER_ENV_PREFIX` + `workerSource()` — the one place the Node-hiding prefix is applied, used by both Worker paths (`workerEmbeddingProvider.ts`, `workerBundleUrl.ts`, whose file is now fingerprinted by content) and deliberately not by the iframe. `frame/model.ts`: `embed(string)` → `embedBatch(string[])` with `padding: true`, and the `numThreads` guard gained a reporting `else`; `frame/entry.ts` batches at `EMBED_BATCH_SIZE = 16`. `EmbeddingProvider` gains `backend()` and the `EmbeddingBackend` type; `createEmbeddingProvider` gains an `onBackend` callback, logged in `main.ts` and shown in the vault-index status line (`vaultIndexBackend`). `VaultIndexService.doSync` persists every `PERSIST_EVERY_EMBEDS = 25` — no more often than `MIN_PERSIST_INTERVAL_MS = 30_000`, since each write serializes the whole index — and unconditionally before rethrowing; `persist` assigns `this.items` as well as writing, so an in-place retry resumes rather than re-embedding; `snapshot()` reads the pre-loop `existing` map, never the live field. A failing note is skipped, but `MAX_CONSECUTIVE_EMBED_FAILURES = 5` in a row rethrows, and the streak resets on a reused note too. The constructor gains `persistIntervalMs` as a test seam. New pure `services/embedding/host/frame/batchSlice.ts` (`sliceBatch`), split out of `model.ts` so the short-batch guard is testable at all — `model.ts` imports transformers at module scope. `VaultRagService.refresh({ force })` builds only once; the send path no longer rescans the corpus, and `reindex` is the one caller that forces. `ui/embeddingSettings.ts` takes a `registerCommit` callback and routes `vaultContextMaxIndexedNotes` through `bindNumberSetting` — the last numeric field outside it. +28 tests (1294 across 88 files); new suites: `batchSlice`, `embeddingProviderFactory` (fallback order + backend reporting), `vaultRagService`, `embeddingSettings`. `models/embeddingModels.ts` gains `embedChunkChars` (reads the previously-dead `maxTokens`), used by `VaultRagService` for the vault index only. +20 tests (1266 across 84 files).*
...
*Previously: 2026-09-18 — a shared label is not a shared number (ADR-176). `RelatedSimilarity` → `SimilarityPreset` (+ `SIMILARITY_PRESETS`, `DEFAULT_SIMILARITY_PRESET`) across `models/embeddingModels.ts`, `models/settings.ts`, `services/persistence.ts`, `services/embedding/relatedConversations.ts` and `ui/embeddingSettings.ts`: the type named the measured related-conversation floors while also typing the unmeasured vault-retrieval preset. The resolvers (`relatedMinScore`, `vaultRetrievalMinScore`) and `relatedFloors` keep their names — that is where the difference lives. Pure rename; no behaviour, no persisted key, no migration.*

*Previously: 2026-09-18 — Enter writes a line break in the composer (ADR-175). New pure `ui/composerKeys.ts` (`composerKeyAction`: Enter/Shift+Enter → insert, Cmd/Ctrl+Enter → send, never a send while an IME is composing; `composerPlaceholder(isMobile)`), wired in `sidebar.ts`'s one composer keydown handler. `i18n.getLocale` now reads the locale through the guarded `getObsidianLocale` instead of a second, unguarded `window` access that threw for any headless caller of `t()`. +9 tests (1211 across 80 files).*

*Previously: 2026-09-18 — the limit is measured in bytes, and the list is paged (ADR-174). New pure `services/storageSize.ts` (`storageLevel`, `formatBytes`, `STORAGE_WARN_BYTES` 25 MB / `STORAGE_HIGH_BYTES` 50 MB, measured) and `scripts/bench-store.mjs` (the whole-file rewrite, startup parse and list work per vault size). `PluginDataStore`: `dataJsonPath()` extracted (the watcher shares it), `dataFileBytes()`, and a once-per-load `Notice` at `high`; `ui/conversationCapSetting.ts` prints the size under the history limit. `DEFAULT_SETTINGS.maxConversations` 200 → 450, with an `applySettingsMigrations` step moving vaults still on the old default. `HistoryController`: `forkCounts` (counts) becomes `forksBySource` (the conversations), killing the per-row `all.filter` that made the browse listing O(n²), and the listing pages at `BROWSE_PAGE_ROWS = 50` behind a `.p-history-more` row that appends. +11 tests (1203 across 79 files; `tests/conversationCap.test.ts` split out of `tests/persistence.test.ts` under the ADR-097 ratchet).*

*Previously: 2026-09-18 — the delete dialog offers the archive (ADR-173). `DeleteConversationModal` takes a `DeleteConversationActions` object (`onDelete`, `onArchive`, `archiveFolder`) instead of one callback; `HistoryController.deleteConversationWithConfirm` shares one `remove()` between them and archives first, deleting only when the note was written. New `ConversationService.archiveConversation(conv): Promise<boolean>` (+ `plugin.archiveConversation` facade) — false means the note is not on disk, so the caller must not delete. `archiveFolderOf(settings)` in `services/conversationArchive.ts` is now the one folder resolution, read by the eviction, the limit's dialog and the delete dialog. +4 tests (1192 across 77 files).*

*Previously: 2026-09-18 — the limit archives before it deletes (ADR-172). New `services/conversationArchive.ts` (pure `archiveNotePath` — never returns a taken path —, `archiveNoteContent`: frontmatter + full transcript, and `archiveFolderOf`) and `NoteWriter.archiveConversationNote`, called from the new `PluginDataStore.applyCap`, which archives first and drops only what was written (a failed write keeps the conversation; every outcome raises a Notice; an `evicting` flag makes it re-entrancy-safe). `persistence.ts`: `partitionEvictions` is now the one rule, with `evictConversations`/`countEvictions` expressed through it. New settings `archiveBeforeEviction` (default ON) + `archiveFolder` (`Pythia/Archive`). New `ui/conversationCapSetting.ts` owns the history-limit field, its dialog and the empty-box rule (`capFieldValue`: `0` is stored, an EMPTY field is shown — same for the message cap). `settings.ts` 550 → 528 lines, ceiling ratcheted. +14 tests (1188 across 76 files).*

*Previously: 2026-09-18 — the conversation cap stops deleting on a keystroke (ADR-171). `PluginDataStore.persist({ evict })` defaults to NOT evicting: only `saveConversations()` applies the cap, so a settings or secret write can no longer delete conversations; `activeConversationIds()` extracted, new `pendingEvictionCount(cap)` (plugin facade of the same name). New pure `countEvictions` in `services/persistence.ts`. New `ui/numberSetting.ts` (`parseNumberSetting` + `bindNumberSetting`: every numeric settings field commits on blur/Enter instead of per keystroke, and `PythiaSettingTab.hide()` flushes the field the user is standing in) and `suggest/ConversationCapModal.ts` (lowering the limit names how many conversations it deletes and asks first). `settings.ts` 565 → 550 lines, ceiling ratcheted. +21 tests (1174 across 75 files).*
...
*Previously: 2026-09-17 — measured related-conversation floors and a warm index (ADR-169). `models/embeddingModels.ts`: `EmbeddingModelConfig.relatedFloors` (per-model cosine floors) and the `RelatedSimilarity` type moves here. `relatedConversations.ts`: `relatedMinScore(preset, modelId)` reads them; new `vaultRetrievalMinScore` / `VAULT_RETRIEVAL_MIN_SCORES` keep vault RAG on the original, unmeasured constants. `ConversationIndexService.sync(conversations, { signal })` aborts between conversations and commits partial progress first; `getRelated` forwards the signal. New `services/embedding/warmIndex.ts` (`shouldWarmIndex`, `warmIndex`) and `services/embedding/host/workerBundleUrl.ts` (`embeddingWorkerUrl`, moved out of `main.ts`, which crossed the 600-line ceiling). `main.ts`: `RELATED_RESULT_LIMIT = 20`, a layout-ready background warm, `ensureEmbeddingProvider({ silent })`. `HistoryController` owns an `AbortController` per related query. Measurement tool: `scripts/measure-related.mjs`. +16 tests (1144 across 73 files).*

*Previously: 2026-09-17 — graded matching and the note dimension (ADR-168). New `services/tokenMatch.ts` (`matchStrength` exact/prefix/infix/reverse with length floors, `tokenMatches`, `applyRelevanceFloor`, `RELEVANCE_FLOOR`) and `services/searchScope.ts` (`parseScope` — `note:`/`conv:`/`all:` + German aliases, `shouldWiden`, `WIDEN_MIN_RESULTS`). `conversationSearch.ts` replaces `buildConversationHaystack` with `buildConversationFields` (`ConversationFields`: title ×3 · notes ×2 · summary · body), adds `noteRefs` (attached + cited + template paths, deduped), `scopeFields`, `FIELD_WEIGHTS`, and `searchConversations` (the scope + auto-widen decision, returning `primary`/`widened`); `rankConversations` takes fields + a scope and reports `matchedNotes`. `HistoryController` memoizes fields instead of tokens and renders the widened group, the `via …` sub-line and the widen chip; `ConversationSuggestModal` shares the same entry point. +50 tests (1128 across 72 files).*

*Previously: 2026-09-16 — ADR-167 addendum. `ui/keyboardInset.ts`: `ViewportMetrics.keyboardHeight`, `readKeyboardHeight()` (Obsidian's `--keyboard-height`), `keyboardOverlap` takes the larger of the viewport and keyboard-height estimates, `watchViewport` also listens to Obsidian's `keyboardWillShow`/`keyboardWillHide` and re-measures after `KEYBOARD_SETTLE_MS`; `HistoryController` passes the same number; `ui/keyboardInset.ts` joins the ESLint window-listener allow-list. `styles.css` gains the composer's desktop clearance (`var(--s3)`) and its phone-drawer override (`var(--s1)`).*

*Previously: 2026-09-16 — glossary definitions in the conversation's language (ADR-166). New `services/languageDetect.ts` (`detectLanguage`). `glossaryNotes.ts`: `definitionKey`, `definitionHash`, `cachedTranslation`, `applyTranslation`, `definitionLanguageOf`, `displayLanguage`, `needsTranslation`; `entryFromFrontmatter` reads `language`, `definition_<lang>`, `translated_from`. `GlossaryEntry` gains `language`, `definitionTranslations`, `translatedFrom`. `GlossaryService.translate(entry, lang)` (cache → in-flight → session map → `llmRouter.translateDefinition` → `processFrontMatter(applyTranslation)`); new entries record `language`. `BaseProvider.translateDefinition` and the private `definitionLanguage` line used by `defineTerm`/`describePerson`. `GlossaryController.show` picks the target (`resolveLanguageState` → `displayLanguage`, stored on the anchor as `data-lang`), renders a pending placeholder and then the translation. +23 tests (1069 across 69 files).*

*Previously: 2026-09-16 — ADR-167 (CSS only): three leaf-scoped rules in `styles.css` and `tests/leafInset.test.ts`, a happy-dom cascade test that stacks core-shaped, plugin and theme-shaped sheets in Obsidian's order. +5 tests (1034 across 67 files).*

*Previously: 2026-09-16 — header instructions (ADR-165). New `ui/instructionState.ts` (`resolveEffortState(conv, globalEffort)` → `{ level, pinned, supported }` via `parameterSupport`; `resolveLanguageState(convLanguage, globalLanguage, obsidianLocale)` → `{ setting, code, pinned, instructed }`) and `ui/choicePicker.ts` (`openChoicePicker` — anchored popover on desktop, `ActionSheet` on mobile; `placeBelow`, now also used by the model popover). `HeaderController`: `updateModelBadge` → `updateInstructions` (paints model | effort | language), `onModelBadgeClick` → `openConversationSettings`; effort and language pickers write `conv.effort` / `conv.outputLanguage` (default row → `undefined`), paint, then save; rename and copy link moved from buttons to the `⌄` menu. `ActionSheetItem` gains `detail?` and `active?`. `main.ts` `onSettingsTabClosed()` (flush + `view.refreshInstructions()`) replaces the settings tab's direct flush. `sidebar.ts` 1731 → 1730 (ratchet lowered). +17 tests (1046 across 68 files).*
...
*Previously: 2026-09-16 — no price table (ADR-163 addendum 3): `settings.priceOverrides`, `resolvePricing` and `sanitizePriceOverrides` removed; `estimateCost`/`costSnapshot`/`messageCost`/`conversationCost` take no overrides; `ui/pricingSettings.ts` renders the toggle and the disclaimer (models.dev linked) only. Earlier: cost snapshot + price pipeline (ADR-163 addendum 2). `Message.cost?: MessageCost { usd, asOf }` written by `sidebar.ts` at stream completion via `costSnapshot`; `messageCost(msg, overrides)` prefers it over a live estimate and feeds `appendTokensToTurnLabel(label, usage, cost?: CostContext)` and `conversationCost`; guarded in `sanitizeMessages`. `scripts/update-pricing.mjs` (`readCatalog`, `buildTable`, `renderTable`, `spliceGenerated`, `UPSTREAM_IDS`) rewrites the GENERATED block of `models/modelPricing.ts` from models.dev; `.github/workflows/update-pricing.yml` runs it weekly and opens a PR; `npm run update:pricing` is the manual path (AGENTS.md release step 0). `showCost` defaults to false. Earlier: price overrides (ADR-163 addendum). `PythiaSettings.priceOverrides: PriceOverrides` (per model `{ input?, output? }`), validated by `sanitizePriceOverrides` inside `mergeSettings` (the one object-valued setting). `resolvePricing(model, overrides)` applies them; `estimateCost`/`conversationCost` take `overrides`; `turnLabel` passes them through `TurnLabelOptions.priceOverrides`. New `ui/pricingSettings.ts` renders the *Show cost* toggle, the disclaimer and one row per catalog model. Earlier: cost per answer (ADR-163). New `models/modelPricing.ts` (`MODEL_PRICING` USD per million tokens with `PRICING_AS_OF`; `estimateCost(model, usage)` → number | null; `formatCost`; `conversationCost(messages)` → { usd, priced, unpriced }). `ui/turnLabel.ts`: `renderTurnLabel(row, msg, conv, { showCost })` and `appendTokensToTurnLabel(label, usage, costModel?)` add `.p-turn-cost`. `PythiaSettings.showCost` (default true; validated by `mergeSettings` through `DEFAULT_SETTINGS`). `HistoryController` row sub-line sums per conversation; `ComparisonController` prices each tab. The next-send estimate (`sendEstimateEl`, `estimateTokensFromText` in the view) is removed; `sidebar.ts` 1755 → 1733. +11 tests (1008 across 64 files).*

*Previously: 2026-09-16 — token-limit support (ADR-162). `RoundResult.truncated` from all three providers; `BaseProvider.streamMessage` passes `StreamFinish { truncated }` as `onComplete`'s third argument (through `LLMProvider`/`LLMRouter`); `Message.truncated?: true` persisted and guarded in `sanitizeMessages`. New pure `services/settingsAdvice.ts` (`maxTokensAdvice` → clear | pin | null, `effectiveMaxTokens`, `raisedMaxTokens`, `isThinkingModel`) and `services/conversationEdits.ts` (`spliceExchange`, now also used by the delete bar). New `ui/SendHintController.ts` (the warning beside Send, moved out of `sidebar.ts`; announces once per conversation-and-model on mobile) and `ui/TruncationController.ts` (the card under a cut-off answer: Continue / Retry with raised limit / Compare; `noticeEmptyReply`). `MODEL_PROFILE` + `profileLine` in `models/modelGuidance.ts`, read by the header popover and `ModelSuggest`. `sidebar.ts` gains `sendText(text)` (send keeping the draft) and shrinks 1759 → 1755. +30 tests (997 across 63 files).*

*Previously: 2026-09-16 — second review (ADR-161). New `ui/outsideDismiss.ts` (`attachOutsideDismiss`: deferred outside-press / touch / Escape dismisser with a dispose-safe registration) used by the model popover, the navigator, the fork menu, the Send menu and the history panel; `ui/longPress.ts` `onFire` receives the press point and gains `touchOnly` (the history rows use it). `services/noteRelevance.ts` gains `scoreRelevanceTokenSets`; `rankConversations` accepts token arrays; `HistoryController` and `InlineSuggest` cache token sets. `PluginDataStore.saveSettingsSoon()` / `flushSettingsSave()` coalesce typed settings; `plugin.saveSettingsSoon()` facade. `models/knownModels.ts` gains `parameterSupport(provider, model)` (settings tab + conversation modal). `GlossaryService.pathFor(entry)`; `glossaryNotes.entryFrontmatter` writes a `term` property when the file name had to be sanitized. `deserializeIndex` bounds-checks. `webSearchHeuristics` compiles its cue patterns once. ESLint confines raw `document`/`window` listeners to an allow-list. +18 tests (967 across 60 files).*

*Previously: 2026-09-16 — model comparison (ADR-160). `models/types.ts` gains `Comparison`/`ComparisonCandidate` and `Conversation.comparison?` (additive). New pure `services/comparison.ts`: `startComparison` (the last answer leaves `messages` and becomes candidate 0), `addCandidate`/`removeCandidate`, `keepCandidate` (appends the kept message, returns a `ForkSpec` per other candidate, moves favorites/merges with their answer), `cancelComparison`, `forkNameFor`, `normalizeComparison` (load-time guard, wired into `parseConversations`). `ConversationService.createComparisonFork(source, spec)` creates the fork conversation. New `suggest/ModelSuggest.ts` (keyed catalog models minus those already answering). New `ui/ExchangeActionsController.ts` (the last-bubble long-press bar: delete · ⇄ compare · cancel — moved out of `sidebar.ts`) and `ui/ComparisonController.ts` (card with a tab per model, sequential candidate runs through `llmRouter.streamMessage` with `writeMode: "none"`, keep/discard). `sidebar.ts` blocks `sendMessage` while `conv.comparison` is set and renders the card after the messages on a full rebuild; 1853 → 1763. +26 tests (949 across 59 files).*

*Previously: 2026-09-16 (follow-up) — Tavily key moves from the JSON body to an `Authorization: Bearer` header in `services/WebSearchService.ts`, with 401/403/429 mapped to specific tool-result errors; `ui/longPress.ts` gains `preventTouchDefault`, and the delete-exchange gesture in `sidebar.ts` is the fourth caller of the shared helper (1885 → 1857). +4 tests (930 across 58 files).*

*Previously: 2026-09-16 — quality & security review (ADR-159, engineering-review #124–#178). `services/persistence.ts` gains `sanitizeConversationFields` and a validating `mergeSettings` (per-key type check, enum lists, unknown keys dropped). `services/pathUtils.ts` gains `safeNoteName`, `normalizeVaultPath`, `yamlString`; `utils.ts` gains `resumeDeepLink` and a local-date `todayISO`. `NoteWriter.createNote` (never overwrites) backs the `create_note` tool. `services/messageUtils.ts` gains `parseToolArguments` and `cleanGeneratedTitle`. Anthropic/OpenAI clients run with `maxRetries: 0` (one retry policy, `runStreamRound`'s). New `ui/emptyState.ts` (`renderNoConversation`, `renderWelcome`) under the ADR-097 ratchet: `sidebar.ts` 1891 → 1885. `tsconfig.json` is `strict`; ESLint enforces `eqeqeq`, the ADR-139 date-formatter ban and no HTML-string injection. +46 tests (926 across 58 files).*

*Previously: 2026-09-15 — `callUtility` reads every text block (ADR-158). `AnthropicService.callUtility` filters `response.content` to `type === "text"` and joins, instead of type-checking `content[0]` — a response is a list and a `thinking` block leads it on a reasoning model, which returned `""` and made the favorites summary render no card. `MistralService.callUtility` gained the same handling for chunk-list content. `SummaryController.runFavoritesSummary` now shows a Notice on an empty result (`summaryEmpty`) rather than returning silently, and writes the summary to `conversationStore.getById(conv.id) ?? conv` so the object `renderSummaryCards` re-reads is the one that got it. +5 tests (880 across 56 files).*

*Previously, 2026-09-15 — people are glossary entries (ADR-151). `GlossaryEntry.kind: "term" | "person"` (undefined = term); `TermIndex` gains `kinds` and `entryKind(index, surface)`, so one alternation matches both and the painter picks the mark. `glossaryNotes.ts` gains `PEOPLE_SUBFOLDER`/`PERSON_TYPE`, `termPath(root, term, kind)` and `subfolderFor`; `type` in frontmatter is the kind. `GlossaryService` reads `Terms/` and `People/`, and gains `lookupPerson`/`describeAndStore`. New `BaseProvider.describePerson` (passage first, outside knowledge only if confident, decline rather than guess) through `LLMProvider`/`LLMRouter`. `HighlightPainter` paints `<pythia-person class="p-person">` (solid faint underline); the anchor is `.p-term-anchor` + `.p-term-anchor--person` (double left rule). New pure `ui/entitySelection.ts` holds the selection rule both Define and Person use — extracted because it paid `SelectionController`'s ADR-097 budget. +16 tests (839 across 54 files).*

*Previously, 2026-09-15 — one note per term (ADR-150). New pure `services/glossaryNotes.ts` owns the folder format: paths (`<root>/Terms/<Term>.md`, `<root>/Themes/<Theme>.md`), `entryFrontmatter`/`entryFromFrontmatter`, `renderBody`/`parseBody`, `mergeEntry` and `effectiveTheme`. `GlossaryService` rewritten around it: `all()` builds entries from `metadataCache` frontmatter and **reads no file**, `hydrate()` reads the one term being opened, `save()` merges into the existing note and writes properties through `fileManager.processFrontMatter`, plus `ensureThemeNote`, `renameTheme` and `migrateLegacyNote`. `services/glossary.ts` keeps `parseGlossary` (migration reader) and `buildTermIndex`; `renderEntry`/`upsertGlossaryEntry` deleted. `GlossaryEntry` gains `theme: string[]`, `context` → `contexts: string[]`. `Conversation.theme?` (undefined = follow the name) with `PythiaSettings.glossaryFolder`; `ConversationService.renameConversation` is the single rename path (all four call sites rewired) and forks pin `effectiveTheme(source)`. `NoteWriter.ensureFolder` is public; new `ui/glossarySettings.ts` (ADR-097 ratchet: `settings.ts` 622 → 597). +34 tests (823 across 53 files).*

*Previously, 2026-09-15 — the glossary note is the interchange format (ADR-149). `GlossaryEntry` gains `translations: Translation[]` (language-tagged equivalents, ISO 639-1 code + term) and `context` (one verbatim sentence from the passage); `aliases` narrows to same-language forms. `services/glossary.ts` writes them as **visible** labelled markdown (`**Forms:**`, `**Translations:**`, `**Context:**`, separator `·`) below the definition, leaving only `source`/`updatedAt`/`model` in the `%% pythia: … %%` comment — a field inside an Obsidian comment is invisible to every external reader. `buildTermIndex` registers translations as matchable surfaces, so an Italian answer marks "contatore" and opens the entry filed under "Zähler"; `TermSource` gains `translations`. `parseDefinitionAndVariants` → `parseDefinitionReply`, now returning `{ definition, variants, translations, context }` and splitting the definition at the earliest marker line; new `TRANSLATIONS_MARKER`/`CONTEXT_MARKER` in `promptConstants.ts`, and `defineTerm` carries ISO 704's definition rules with its budget raised 300 → 420. Legacy `aliases=` in the comment is still parsed; entries upgrade lazily on re-definition. +16 tests (797 across 52 files).*

*Previously, 2026-09-15 — one language setting, resolved per conversation (ADR-148). `outputLanguage` is now `OutputLanguage` (`"obsidian" | "auto" | "de" | "en" | "it" | "es"`, `models/types.ts`) and feeds the chat answer as well as the utility prompts: `buildSystemPrompt` takes an `opts.languageLabel` and pushes `langDirective(label)` after the `<custom_instructions>` block. `Conversation.outputLanguage?` overrides the global setting (undefined = inherit), edited in `ConversationSettingsModal`; `BaseProvider.languageLabel(conversation?)` is the single resolver every prompt in the class goes through. `services/messageUtils.ts` gains `LANG_LABELS` (widened to Obsidian's ~30 UI locales), `languageLabelForLocale`, `resolveLanguageLabel` and `langDirective` — `langInstruction`/`langSuffix` now take the resolved label, not the setting code. `i18n.ts` gains `getObsidianLocale()`; new `ui/languageOptions.ts` owns the shared dropdown list. `defineTerm`/`generateChapterName`/`generateConversationTitle`/`summarizeNotes` gained an optional trailing `conversation?` (mirrored on `LLMProvider`/`LLMRouter`). +16 tests (781 across 52 files).*
...
*Previously, 2026-09-15 — conversation picking reuses the history panel (ADR-143). `HistoryController.openHistoryView(pick?: HistoryPick)` gained an optional pick mode (`{ excludeId, placeholder, onPick }`); `PythiaSidebarView.pickConversation` is the thin facade plugin commands reach it through, and `ConversationService.cmdMergeConversation` now calls it instead of opening a `ConversationSuggestModal`, with the link-recording work split into `linkToConversation`. `ConversationSuggestModal` remains for the command-palette entry points (`cmdBrowseConversations`, resume), which can run with no view open. Tests restructured: new `tests/helpers/viewHarness.ts` (mount fixture) and `tests/historyPanel.test.ts` (ADR-107/109/143), taking `tests/viewRender.test.ts` from 650 to 331 lines and clearing the last grandfathered ceiling in `scripts/check-file-size.mjs`. +8 tests (766 across 51 files).*

*Previously, 2026-09-15 — summary contract and clamp (ADR-141). `services/BaseProvider.ts` gained a module-level `SUMMARY_RULES` shared by `generateSummary` and `generateSummaryWithTitle`, which previously duplicated their instructions; token caps unchanged. New `ui/clampBody.ts` (`clampSummary(body, mount)`) bounds the fork and merge anchor summaries and is wired into both controllers. New `tests/clampBody.test.ts`; +6 tests (758 across 50 files).*

*Previously, 2026-09-15 — template moved into the sources row (ADR-140). `renderSourcesRow(app, row, sources, templatePath?)` gained a fourth argument and renders a TEMPLATE row first; `ui/turnLabel.ts` no longer renders `.p-turn-template` but still exports `turnTemplateCaption`, which `sidebar.ts` now calls to decide the template shown under each answer (and resolves once up front for the streaming row). A shared `renderWikilink` backs both the template and the vault citations. New `tests/sourcesRow.test.ts`; +14 tests (752 across 49 files).*

*Previously, 2026-09-15 — date formatting consolidated (ADR-139). New `formatDate` and `formatMonthYear` in `services/messageUtils.ts` — locale-independent, like `formatClockTime` — are now the only UI date formatters: `formatSummaryTimestamp` composes `formatDate` + `formatClockTime`, `turnLabel.formatTurnDate` delegates to `formatDate`, and `HistoryController.historyBucket` uses `formatMonthYear`. No `toLocaleDateString` or `toLocaleTimeString` remains in the UI; `NoteWriter`'s ISO stamps are unaffected (file data, not display). +5 tests (743 across 48 files).*

*Previously, 2026-09-15 — glossary aliases (ADR-137). `GlossaryEntry` gained `aliases`: the inflections, plurals and cross-language equivalents of a term, matched and marked exactly like the term itself and resolving back to the same entry. `buildTermMatcher(terms)` is replaced by `buildTermIndex(entries): { matcher, canonical }` — with aliases the matched surface form is no longer the term, so `repaintTerms` writes the *canonical* term into `data-term` (via `canonicalTerm`) and the anchor still resolves. `GlossaryService.matcherFor` → `indexFor`; `find` and `remove` match aliases too, so re-defining an inflected form hits the cached entry instead of the model. `defineTerm` now asks for a structured `DEFINITION:` / `VARIANTS:` reply, read back by the new `parseDefinitionAndVariants` in `services/messageUtils.ts` (markers in `promptConstants.ts`, same shape as `TITLE:` / `SUMMARY:`). Aliases are stored last inside the existing `%% pythia: … %%` marker, pipe-separated. +15 tests (738 across 48 files).*

*Previously, 2026-09-15 — glossary (ADR-136). New `services/glossary.ts` (pure: `parseGlossary`, `upsertGlossaryEntry`, `normalizeTerm`, `buildTermMatcher`), `services/GlossaryService.ts` (vault note I/O, vault-then-model lookup chain, cache invalidated by our own writes and by hand edits via the vault watcher in `main.ts`), `ui/GlossaryController.ts` (term marks + inline definition anchor), and `repaintTerms` in `ui/HighlightPainter.ts`. `defineTerm` runs as a utility call through `BaseProvider` → `LLMProvider` → `LLMRouter`, so it never enters the message list. `PythiaSettings` gained `glossaryNote`; `AppContainer` constructs `GlossaryService`. **No new `Conversation` field**: the glossary note is the only source of truth and terms are matched, not stored. Two extractions paid for it under the ADR-097 ratchet — `ui/citationPainter.ts` out of `sidebar.ts` (1,891 → 1,877 lines, ceiling lowered) and `withConversationBacklink` in `utils.ts` replacing a duplicated deep-link builder in `SelectionController`, which brought that file back under the 600-line default. +33 tests (723 across 48 files).*

*Previously, 2026-09-15 — ADR-135. `ui/keyboardInset.ts` gains `watchViewport(onChange)`, which owns the `visualViewport` listeners and returns a disposer; `sidebar.ts` also subscribes to the workspace's `resize` and `layout-change`, because a leaf opening or closing moves the panel's bottom edge without firing any viewport event and would otherwise leave `--p-bottom-inset` stale. `sidebar.ts` 1,903 → 1,891 lines, ceiling lowered.*

*Previously, 2026-09-15 — ADR-134 corrects ADR-131 and ADR-132, neither of which fixed what was reported. `ui/keyboardInset.ts` gains `needsBottomSafeArea` and `updateViewportInsets`, which now owns both viewport-derived insets (keyboard lift plus the conditional home-indicator padding) so `sidebar.ts`'s `adjustForKeyboard` is a one-line delegate; `sidebar.ts` 1,912 → 1,903 lines, ceiling lowered. The table fix is CSS only: `width: max-content` plus a per-cell `max-width: 32ch`. +4 tests (694 across 46 files).*

*Previously, 2026-09-15 — conversation data-loss fix (ADR-133). `services/persistence.ts` gains the pure `mergeConversations(memory, disk)`; `PluginDataStore.loadPluginData` now reconciles with it instead of `p.conversations = loaded`, so a stale data.json can no longer roll a conversation back to an older state. `reloadFromDisk` flushes whatever memory won so disk catches up. The watcher's own-write stamp is taken after `saveData` resolves as well as before, and its mtime baseline is seeded from the file rather than the clock. +10 tests (690 across 46 files; `tests/persistence.test.ts` was split, with the new `tests/mergeConversations.test.ts` keeping it under the 600-line budget).*

*Previously, 2026-09-15 — keyboard avoidance corrected (ADR-132). New `ui/keyboardInset.ts` holds the decision as a pure, unit-tested function (`keyboardOverlap`, `MIN_KEYBOARD_INSET`) plus a thin DOM shell (`currentKeyboardOverlap`). `sidebar.ts`'s `adjustForKeyboard` now does nothing unless a real soft keyboard is open, and lifts content with `padding-bottom` instead of setting an explicit `height` — the latter was uncovering a 67px strip of leaf beneath the panel and clipping the input area's own safe-area padding, since `.pythia-view` is `overflow: hidden`. +8 tests (681 across 45 files).*

*Previously, 2026-09-15 — wide tables scroll instead of squeezing (ADR-131), plus three extractions out of `sidebar.ts` and `CodeBlockDecorator.ts`. New `ui/tableDecorator.ts` (`decorateTables`, idempotent table wrapping), `ui/dragToPan.ts` (`attachDragToPan`, moved out of `CodeBlockDecorator` and now shared by code blocks, diagrams and tables) and `ui/renderMarkdown.ts` (`renderRichMarkdown`, replacing three identical `MarkdownRenderer.render` callbacks in the view so summary cards and the fork/merge anchors get the same table treatment assistant messages do). The fix itself is CSS: the `.p-scroll-frame` wrapper already existed but had no effect, because a table sizes itself to its container rather than overflowing it. `sidebar.ts` 1,920 → 1,912 lines, ceiling lowered to 1,912. +8 tests (673 across 44 files).*

*Previously, 2026-09-14 — Merge links (ADR-130), the inverse of Fork. New `ui/MergeController.ts` owns the merge-link display surfaces: the painted `<pythia-merge class="p-merge-link">` marks, the inline `.p-merge-anchor` they open (target name + its conversation summary + regenerate/unlink/open), the `.pythia-merge-banner` shown at the top of the conversation a link points at (derived on read via the pure, exported `incomingMergeLinks`, so there is no stored back-reference to keep consistent), and removal. `ui/HighlightPainter.ts` gained `repaintMergeLinks` over the same find/paint machinery. `models/types.ts` gained `MergeLink` and `Conversation.merges?` (additive, backfill-safe). `services/persistence.ts` gained `normalizeMerges` (read-path guard) and `evictConversations` now protects merge targets. `services/ConversationService.ts` gained `cmdMergeConversation` (conversation search → summary-if-missing → link → reveal). Nothing was added to `ContextBuilder` — a merge is display-only and never enters the system prompt. Two extractions paid for the new wiring under the ADR-097 ratchet: new `ui/longPress.ts` (one tested 450 ms press-and-hold, replacing three hand-rolled copies in `sidebar.ts` and `ForkController`), `unwrapCodeFence` moved into `services/messageUtils.ts` as a pure function, and `applyAccentContrast` moved into the new `ui/accentContrast.ts` (keeping `services/color.ts` DOM-free) — `sidebar.ts` 1,951 → 1,920 lines, ceiling lowered to 1,920. +35 tests (665 across 43 files).*

*Previously, 2026-09-12 — turn labels extracted and the template moved onto them (ADR-129). New `ui/turnLabel.ts` owns `renderTurnLabel` / `appendTokensToTurnLabel` / `turnTemplateCaption` / `isFirstMessageOfDay` / `formatTurnDate` as pure `(row, msg, conv)` functions — no view state — so the label rules are unit-testable without mounting the view (+8 tests, 630). New `services/pathUtils.ts` (`noteBasename`) replaces the `split("/").pop().replace(/\.md$/,"")` pattern duplicated across `citations.ts`, `ContextInspectorController`, and `sidebar.ts`. `models/types.ts` `Message` gained `templateId?` (additive, backfill-safe — it falls back to `Conversation.templateId`), stamped on the assistant message at generation time. `ui/HeaderController.ts` lost the template caption entirely. Under the ADR-097 ratchet: `sidebar.ts` 2,018 → 1,951 lines, ceiling lowered to 1,951.*

*Previously, 2026-09-10 — `lastTokenUsageMsg` moved out of `sidebar.ts` into `services/messageUtils.ts` as the pure, generic `lastTokenUsageMessage(messages)` (+3 tests). Both call sites — the send-estimate label and the context inspector's `getLastTokenUsageMsg` dep — pass the active conversation's messages. This paid for the streaming toolbar change under the ADR-097 ratchet: `sidebar.ts` 2,028 → 2,018 lines, ceiling lowered to 2,018.*

*Previously, 2026-09-04 — embedding off-thread on capacitor:// builds (ADR-126): `blob:` Workers are blocked there, so the plugin writes the embedding bundle to `<pluginDir>/embedding-worker-<version>.mjs` once and starts the Worker from `adapter.getResourcePath(path)` (same-origin, blob-free). `WorkerEmbeddingProvider` takes an injected `spawnUrl`; the fallback chain is now blob Worker → resource-path Worker → iframe. Restores off-thread inference (no UI freeze) where blob is blocked; falls back safely to ADR-125's throttled UI-thread build otherwise.*

*Previously, 2026-09-04 — vault RAG UI-thread throttle (ADR-125, supersedes ADR-124): the real constraint is whether the embedding Worker engaged, not `Platform.isMobile` — blob-URL Workers are blocked on mobile AND some desktop builds, forcing the UI-thread iframe. Providers now expose `isOffThread()`; `VaultRagService.refresh()` reads it after `ready()` and, when on the UI thread, serves an already-built persisted index via `hydrateForQuery()` (no rebuild) or builds a new one THROTTLED (`VaultIndexService.sync` `{yieldEveryNotes,breatherMs}` → yield every note + 12 ms breather) so the app stays responsive. `applyChanges`/`reindex` are no longer platform-gated. +1 test.*

*Previously, 2026-09-04 — vault RAG mobile fix (ADR-124): Obsidian mobile (Capacitor) blocks `blob:` Workers, so embedding falls back to the UI-thread iframe; a full vault build there freezes the app. `VaultRagService` now gates on `isMainThreadOnly` (`Platform.isMobile`): mobile `refresh()` calls the new `VaultIndexService.hydrateForQuery()` (load persisted index + mark queryable, no note embeds), `applyChanges` is skipped and `reindex` refused (one-time Notice). The index `.bin` lives in the plugin dir so a desktop-built index syncs to mobile, which queries against it (one embed/turn). +2 tests.*

*Previously, 2026-09-04 — vault RAG batch persistence (ADR-122): `VaultIndexService.applyBatch({updates, removes}, {cap})` mutates the index in memory (via `updateInMemory`/`removeInMemory`) and serializes + writes the packed `.bin` AT MOST ONCE per batch; `updateNote`/`removeNote` are thin single-item wrappers over it; `VaultRagService.applyChanges` builds one batch per debounced watcher flush → one index write instead of one per note. The analogous `data.json` cost (whole-file rewrite per conversation save) was investigated as a per-conversation store and REJECTED — splitting endangers the load-bearing Desktop→Mobile iCloud sync (ADR-010/011) for a small local write win; see ADR-123 in `docs/decisions.md`.*

*Previously, 2026-09-04 — vault RAG perf (ADR-121): event-driven indexing + one ML bundle. The watcher now batches changed `TFile`s and calls `VaultRagService.applyChanges` → `VaultIndexService.updateNote`/`removeNote` (targeted: one embed per edited note, vs a full rescan); a new `enqueue` op chain serializes targeted updates against full builds; `size()` added. Embedding bundle unified: one `frame/entry.ts` (detects worker vs iframe at runtime) built once and inlined once via `embeddingBundle.getEmbeddingBundle()` (lazy getter); iframe/worker providers share it; `bootstrap.ts`+`worker.ts` deleted; esbuild builds one embedding bundle; `globals.d.ts` → `__EMBEDDING_BUNDLE_PLACEHOLDER__`. `main.js` 2.5 MB → 1.6 MB. New tests: targeted-update cases in `VaultIndexService.test.ts`.*

*Previously, 2026-09-04 — vault RAG scale hardening (ADR-120): `IndexableNote` is now `{ path, load() }` (lazy); `VaultIndexService.doSync` STREAMS — reads+chunks+embeds one note at a time and releases its text, so peak memory is bounded on huge vaults (was: whole-vault content held at once). `query` ranks cooperatively (yields every 2000 notes) so a large corpus doesn't block the UI. New pure `services/embedding/indexScope.ts` `selectIndexPaths` (folder scope + note cap); `vaultContextMaxIndexedNotes` setting (default 5000) with a one-time "capped, scope to folders" warning; `VaultRagService.collectIndexableNotes` returns lazy refs + cap info. New tests: `indexScope.test.ts`, streamed lazy-load + progress cases.*

*Previously, 2026-09-04 — vault RAG off-thread + scope + reindex + watcher (ADR-119): embedding now runs in a real Web Worker (`services/embedding/host/frame/worker.ts` + `WorkerEmbeddingProvider`, a blob-URL module worker bundled via `__WORKER_CONTENTS_PLACEHOLDER__`) instead of the main-thread iframe; `embeddingProviderFactory.createEmbeddingProvider` returns a `FallbackEmbeddingProvider` (worker → iframe fallback). `VaultIndexService.sync` yields cooperatively + reports progress; `.clear()` backs reindex. Vault-RAG lifecycle extracted from `main.ts` to `services/VaultRagService.ts` (index build/refresh in the background, `getRelevantNotes`, `getAutoContext`, `reindex`, folder-scoped `collectIndexableNotes`, live status). New `vaultContextFolders` setting; embedding + vault-context settings UI moved to `ui/embeddingSettings.ts` (with a "Rebuild index" button + status). `main.ts` adds a `reindex-vault-context` command and a debounced `vault.on(...)` watcher. `main.js` +~0.85 MB (ML runtime bundled for both iframe and worker).*

*Previously, 2026-09-03 — vault RAG made non-blocking (ADR-118, bug fix): `VaultIndexService` split into `sync(notes)` (build/refresh + `isReady()` flag) and `query(text, {minScore, limit, exclude})` (embeds only the query, ranks the in-memory index — safe inside a turn; excludes filtered after ranking, before the limit). `main.ts.getRelevantNotes` now: kicks `syncVaultIndex()` (a coalesced BACKGROUND whole-vault embed, the only place the vault is read/embedded, failures logged not surfaced) and returns [] immediately while `!svc.isReady()`, else `await svc.query(...)`. Result: a chat turn never waits on the first-use model download or a re-index — the LLM reply is immediate and RAG only adds context once the background index is ready. New i18n `vaultIndexBuilding` (one-time notice); `invalidateRelatedService` resets the notice flag.*

*Previously, 2026-09-03 — vault RAG control + provenance + gating fix (ADR-117): per-conversation `Conversation.vaultContext` flag toggled by a new `library`-icon toolbar button in `sidebar.ts` (`updateVaultButton`/`toggleVaultContext`, mirroring the research globe); `getRelevantNotes(conversation, …)` now gates on `conversation.vaultContext ?? settings.vaultContextEnabled` (the command toggles the default). `buildSystemPrompt` gained a `{ hasAttachedNotes }` option, passed from `BaseProvider.resolveUserContent` (true when note text is actually inlined) so the citation instruction AND the ADR-115 untrusted-content framing fire for RAG-appended notes, not just notes stored in `contextNotes` — fixing a hole where auto-retrieved notes were injected uncited and unframed. `main.ts` records retrieved paths per conversation (`getAutoContext`); the reference row renders them as distinct read-only "auto" pills (`.p-wikilink--auto`), refreshed after each turn. `onCitationClick`+`renderSourcesRow` extracted to `ui/sourcesRow.ts` (`openCitationSource`/`renderSourcesRow`) to keep `sidebar.ts` at its size ceiling.*

*Previously, 2026-09-03 — vault-wide semantic RAG (ADR-116): auto-retrieval of relevant vault notes per turn, reusing the on-device embedding engine. New `services/embedding/vaultRetrieval.ts` (pure: `noteEmbedChunks` + `rankByQuery`) and `services/embedding/VaultIndexService.ts` (mirrors `ConversationIndexService`: incremental `sync(notes)` + `retrieve(query)`), persisted via `VaultIndexStore` under a new `vault-embeddings-<model>.bin` (the store gained a `prefix` param). `main.ts` now shares ONE lazily-built embedding provider between the related-conversations and vault services (`ensureEmbeddingProvider`), adds `getRelevantNotes(query, exclude)` (gathers markdown via `getMarkdownFiles`/`cachedRead`, excludes already-attached + `Conversations/`+`Scratch/`), and installs it as a retriever hook on `LLMRouter.setVaultRetriever`. `LLMRouter.streamMessage` is now async and merges retrieved paths into `attachedNotes` (fail-open) before delegating — so retrieved notes flow through the existing attached-note pipeline (excerpting, token guard, ADR-115 untrusted-content framing, citations) with zero changes to providers or `sidebar.ts`. Gated by new settings `vaultContextEnabled`/`vaultContextMaxNotes`/`vaultContextSimilarity` (default off), toggled by a new command (no settings-tab UI yet — `settings.ts` is at its size ceiling).*

*Previously, 2026-09-03 — prompt-injection & write-tool hardening (ADR-115): `ContextBuilder` gained `neutralizeControlTags()` (defangs Pythia's structural tags inside untrusted note bodies / prior summaries / forked excerpts, `<`→`‹`) and an `escapeAttr` on the `<attached_note>` `path`; `buildSystemPrompt` prepends `UNTRUSTED_CONTENT_INSTRUCTION` (new in `promptConstants.ts`) whenever untrusted context is in play. `ToolHandler.execute` takes a `contextNotes` allow-list arg and enforces it for `rewrite_note`/`prepend_note` (plus a `..`-traversal reject) at the boundary; `sidebar.ts` passes `conv.contextNotes`. `NoteWriter.writeNote` rejects writes into `app.vault.configDir`. `sidebar.onCitationClick` opens only `http(s)` citations via `noopener,noreferrer`. New i18n key `invalidUrl`.*

*Previously, 2026-09-02 — mobile bottom action sheet (ADR-114): new reusable `ui/ActionSheet.ts` (full-width bottom sheet + scrim, swipe/scrim/Escape dismiss, safe-area padding). `sidebar.openSummaryMenu` branches on `Platform.isMobile` — mobile opens the sheet, desktop keeps the `.p-send-menu` popover — both rendered from one shared `buildSummaryMenuItems()` list. New `.p-sheet*` CSS. Long-press trigger unchanged; sheet is the touch surface only.*

*Previously, 2026-09-02 — Upvoty integration removed (ADR-113, reverting ADR-111): the endpoint rejected a valid REST token as `Bearer` (needs OAuth or a differently-presented token; no docs to confirm), so the whole feature was pulled — `services/UpvotyService.ts` + tests deleted, the `upvoty_*` tools/gating/`buildUpvotyArgs` removed from `ToolHandler`, `getToolDefinitions`/`allowedToolNames` back to the `researchEnabled`-only signature, the `upvotyMode` conversation/template fields, the `upvoty*` settings + `SecretStore.setUpvotyKey` + `plaintextUpvotyKey` plumbing, the toolbar toggle, and the i18n strings all gone. The ADR-112 redaction hygiene stays.*

*Previously, 2026-09-02 — secret-redaction hygiene (ADR-112): new pure `services/redact.ts` — `redactSecrets()` masks Bearer tokens, `sk-`/`sk-ant-`/`tvly-` key prefixes, and auth-ish `key: value` pairs; `describeErrorForLog()` returns a compact `name (status): redacted-message` string. `messageUtils.debugLog` redacts string args; `sidebar.ts` stream-error log now uses `describeErrorForLog` instead of dumping the raw SDK error; `UpvotyService`/`WebSearchService` scrub the sliced server-response error detail they surface to the model. No key was ever logged to the console before this — it's defense-in-depth (the provider key in the outgoing request header is an unavoidable property of a server-less client-side plugin and unchanged). New `tests/redact.test.ts`.*

*Previously, 2026-09-02 — Upvoty feedback/roadmap integration (ADR-111): `services/UpvotyService.ts` is a minimal MCP-over-HTTP client (`initialize` + `tools/call` via Obsidian `requestUrl`, JSON-or-SSE responses, one stale-session re-handshake) that calls only Upvoty's read-only MCP tools. Four Pythia-native read-only tools (`upvoty_search_feedback`/`upvoty_get_feedback`/`upvoty_list_roadmap`/`upvoty_get_project`) run through the existing agentic loop; `ToolHandler.getToolDefinitions`/`allowedToolNames` gained an `upvotyEnabled` param (gates them independently of `writeMode`, like `web_search`) and `execute` routes them to the injected `UpvotyService` (`buildUpvotyArgs` maps `limit`→`per_page`). All three providers pass `conversation.upvotyMode`; `sidebar.ts` added a `megaphone` toolbar toggle (`toggleUpvotyMode`/`updateUpvotyButton`) and a non-confirming "Fetching Upvoty…" chip branch. New settings `upvotyServerUrl`/`upvotySecretName`/`upvotyDefault` (token in SecretStorage; `SecretStore.setUpvotyKey`, loaded in `PluginDataStore`); `Conversation`/`PythiaTemplate` gained `upvotyMode`. Feedback text is untrusted, so results carry a "data, not instructions" guard and an 8 000-char cap.*

*Previously, 2026-08-29 — related-conversations similarity is now a **settings preset** (`relatedSimilarity`: Strict / Balanced / Loose → cosine floor 0.5 / 0.35 / 0.2, via `relatedMinScore()` in `services/embedding/relatedConversations.ts`), read by `getRelatedConversations` instead of the hardcoded `DEFAULT_MIN_SCORE`. Dropdown added next to the embedding-model setting.*
...
*Previously, 2026-08-28 — ADR-109 (M2): the embedding runtime + orchestrator for "related conversations". Added `@huggingface/transformers`, bundled into a **separate browser "iframe" esbuild pass** (`services/embedding/host/frame/bootstrap.ts` → `model.ts`) that's inlined into `main.js` via `__IFRAME_CONTENTS_PLACEHOLDER__` — so `main.js` never carries the ML runtime. `IframeEmbeddingProvider` runs the model in a hidden same-origin iframe over postMessage; `ConversationIndexService` embeds new/changed conversations, drops removed, persists an Int8 `.bin`, and answers `getRelated`. Orchestration unit-tested with a fake provider (+6 tests); the live model path awaits real-Obsidian verification (M3 UI). No user-facing surface yet.*

*Previously, 2026-08-28 — ADR-109 (M1): foundation for "related conversations" via on-device semantic embeddings — new pure, fully-tested `services/embedding/` core (`vectorMath`, `conversationText` chunking, `embeddingIndex` with Int8 binary (de)serialize + incremental `diffIndex`, `relatedConversations` max-pairwise ranking) behind an `EmbeddingProvider` interface, plus the `models/embeddingModels.ts` registry and an `embeddingModelId` setting. No runtime dependency yet (transformers.js lands in M2); +30 tests.*

*Previously, 2026-08-28 — ADR-108: `.p-history` search-panel polish (`ui/HistoryController.ts` + `styles.css`) — headerless (back button moved into the search row, "+" dropped), trash-can row delete, and the accent tint moved from the active conversation to the focused row (hover / ↑/↓ selection); the active conversation now shows only a grey `.p-history-active` label. No behavioral change — the ADR-107 smoke tests still pass.*

*Previously, 2026-08-28 — ADR-107: the quick switcher is folded into the history panel (`ui/HistoryController.ts` — `openQuickSwitcher` deleted), leaving one in-view conversation-search surface. The header far-left icon is now a **search loupe** that opens the `.p-history` panel with its search input focused and ↑/↓/Enter keyboard nav; the header **title is plain, non-interactive text** (click + `▾` removed). Covered by four new `tests/viewRender.test.ts` smoke tests (loupe→open→focus, browse-vs-search branch, ↑/↓/Enter nav, inert title) — the fixture gained global `createDiv`/`createEl`/`createSpan`. 466 tests.*

*Previously, 2026-08-28 — ADR-106: conversation search now ranks by lexical TF-IDF over content (title + LLM summary + message bodies) via `services/conversationSearch.ts` + the existing `noteRelevance` scorer. Wired into all three surfaces — the in-panel history view and quick switcher in `ui/HistoryController.ts` (empty query → date-grouped listing; active query → flat relevance-ranked list with a match snippet per row) and the command-palette `ConversationSuggestModal`. Chosen over on-device semantic embeddings (transformers.js/MiniLM, per obsidian-similarity) — documented as the Phase-2 seam.*

*Previously, 2026-08-28 — #125 Tier 2: `tests/viewRender.test.ts` now covers all three `renderMessages` modes — added incremental append (new turn appends without a full rebuild; the original bubble node survives) and delete-last-exchange (splice from model + DOM, welcome-on-empty, and the full-rebuild fallback when the tracked tail id is gone). 451 tests. See ADR-105.*
...
| `services/conversationSearch.ts` | 290 | Lexical conversation search (ADR-106/168): `buildConversationFields` (title · summary · body · notes, tokenized once), `noteRefs` (every attached, cited or template note path, deduped — no vault I/O), `rankConversations` (empty → recency, else IDF × `matchStrength` × `FIELD_WEIGHTS`, score-desc, relevance floor) reporting `matchedNotes`, `searchConversations` (scope + auto-widen → `primary`/`widened`), `bestMatchSnippet` (why-it-matched line) |
...
| `services/searchScope.ts` | 82 | The search box's scope grammar (ADR-168): `parseScope` (`note:`/`conv:`/`all:` + German aliases; an unknown prefix stays literal text), `shouldWiden` + `WIDEN_MIN_RESULTS` — the rule for searching somewhere the user did not ask |
...
| `services/embedding/embeddingIndex.ts` | 115 | Vector index: `IndexedConversation`, FNV content hash, `diffIndex` (incremental add/drop), compact Int8 binary `serialize`/`deserialize` |
...
| `services/PluginDataStore.ts` | ~355 | data.json I/O extracted from the plugin (ADR-103 / #121): `loadPluginData` (migrations + iCloud-eviction guard), `persist({ evict })` (own-write stamp; eviction only when a conversation write asks for it — ADR-171), `applyCap` (archive-then-drop, fail-closed — ADR-172), `saveSettings`/`saveConversations`, `pendingEvictionCount`, the `watchDataJson` cross-device poller, `reloadFromDisk`. Owns `saveDataRecordTime` + the `legacyDecrypt` migration helper; wraps the pure `persistence.ts` |
...
| `ui/HeaderController.ts` | 567 | Header chrome — the header row (search · name · [ctx chip] · model \| effort \| language · menu · delete · new, ADR-165), inline rename flow, model popover, effort and language pickers, and the menu (rename · copy link · settings) — extracted from sidebar (ADR-103 / #120). `mount()` builds the header; `renderHeader`/`updateInstructions` refresh it; `getConvNameEl`/`getChipEl` expose the two elements History/ContextInspector consume; History/ContextInspector/send-hint reached via deps |
| `ui/SelectionController.ts` | 545 | Text-selection toolbar (Copy/Favorite/Branch/Merge/Insert/Inbox) + span-favorites — build/remove/repaint/scroll highlights, the tap-a-highlight interaction, fork-from-selection (`onForkConversation`) and merge-from-selection (`onMergeConversation`, ADR-130) — extracted from sidebar (ADR-103 / #120). `mount()` builds the toolbar and wires the selection listeners via a passed `registerDomEvent`; owns all HighlightPainter usage. The view calls public `repaintFavorites` (render), `scrollToFavorite`/`removeFavorite` (Navigator); a fork-origin tap routes into ForkController and a merge-link tap into MergeController via deps (fork wins over merge, merge over favorites) |
...
| `ui/MergeController.ts` | 232 | Merge-link display (ADR-130) — the inverse of ForkController: the painted `<pythia-merge class="p-merge-link">` marks on passages pointed at another conversation (`repaintMergeLinks` over HighlightPainter), the inline `.p-merge-anchor` those marks open (target name + its conversation summary + `N messages · MODEL · date [· outdated]` + regenerate / unlink / `Open →`), plus `renderIncomingBanner` (the far half of a link: every conversation that merged with this one, with its passage excerpt and a click that opens it and expands the anchor), `revealMergeLink` and `removeMergeLink`. Exports the pure `incomingMergeLinks(conversations, targetId)`, which is unit-tested. `Deps`-driven, same shape as ForkController. Creating a merge from a selection lives in SelectionController + `cmdMergeConversation`. Regeneration uses `generateSummary` (never `generateSummaryWithTitle`) so merging never renames the target |
| `ui/longPress.ts` | 78 | One 450 ms press-and-hold gesture for touch + mouse (`attachLongPress`, `LONG_PRESS_MS`), replacing three hand-rolled copies. Two binding modes: with Obsidian's `registerDomEvent` (it owns removal) or direct listeners (the returned cleanup removes them). Cancels on touchend/cancel/move, mouseup/leave; left button only. Unit-tested with fake timers |
| `ui/accentContrast.ts` | 43 | `applyAccentContrast(root)` — resolves the theme accent and its on-accent tokens through a throwaway probe span and publishes a readable `--p-on-accent` via `readableOnAccent`. Extracted from `sidebar.ts` so `services/color.ts` stays DOM-free and unit-testable; the view only supplies the root element |
...
| `ui/CodeBlockDecorator.ts` | 187 | Code block + diagram decoration extracted from sidebar: `decorateCodeBlocks`, `stampSvgSize`, `fixDiagramSvgSize`, `wrapInScrollFrame`. Delegates tables to `decorateTables` and the pan gesture to `attachDragToPan` (ADR-131) |
...
| `ui/keyboardInset.ts` | 120 | Viewport-derived insets, pure rules plus one DOM shell. `keyboardOverlap` returns how much of the panel a soft keyboard covers and 0 when none is open (ADR-132); `needsBottomSafeArea(containerBottom, layoutHeight)` answers whether the panel actually reaches the screen edge, which is the only way to tell a home indicator from another leaf below (ADR-134); `updateViewportInsets(container)` applies both, since both are driven by the same events. Unit-tested because `visualViewport` and a soft keyboard cannot be reproduced headlessly |
| `services/glossary.ts` | 231 | Pure glossary core (ADR-136/137): `parseGlossary` / `upsertGlossaryEntry` for the `## Term` note format (tolerant, preserves hand edits and the preamble; `aliases` ride last in the `%% pythia: … %%` marker), `normalizeTerm`, `buildTermIndex` — ONE alternation over every surface form of every term, longest-first (JS alternation is first-match, not longest-match) with `\p{L}\p{N}_` lookarounds instead of ASCII-only `\b`, canonical terms registered before aliases so note order cannot decide which entry a word opens — and `canonicalTerm` to map a matched form back to its entry |
| `services/GlossaryService.ts` | 205 | Owns the glossary note: reads and caches entries, builds the `TermIndex` (`indexFor`), resolves a term vault-first — by term OR alias — then via `llmRouter.defineTerm(term, passage)`, parses the `DEFINITION:` / `VARIANTS:` reply and stores the variants as `aliases`, de-duplicates in-flight lookups. Read-modify-write against the file, never the cache, so a hand edit between lookups is not clobbered |
| `ui/GlossaryController.ts` | 162 | Term marks and the inline definition anchor (ADR-136). Unlike Fork/Merge controllers there is no stored span: every occurrence of every known term is marked in every conversation, so `repaintAll` repaints the whole transcript whenever the glossary changes |
| `ui/instructionState.ts` | 67 | Pure (ADR-165): `resolveEffortState` / `resolveLanguageState` — the value a send uses, whether the conversation pinned it, whether the model supports effort. The only source for the header's effort and language segments |
| `ui/choicePicker.ts` | 92 | `openChoicePicker` — one short list of choices for a header control: anchored `.p-choice-pop` on desktop, `ActionSheet` on mobile (ADR-165). `placeBelow` positions it and the model popover |
| `ui/citationPainter.ts` | 43 | `paintCitations(app, body, sources)` — swaps `⟦cite:…⟧` markers for numbered chips. Extracted from `sidebar.ts`; sits with `services/citations.ts` (parsing) and `ui/sourcesRow.ts` (the list) |
| `ui/HighlightPainter.ts` | — | Favorite/fork highlight DOM helpers: `findRange` (re-find stored text across text nodes), `computeOccurrenceIndex`, `paintRange` (wrap a range in a `<pythia-favorite>`/`<pythia-fork>` custom element via the `tagName` param — not `<mark>`, ADR-086 — splitting across element boundaries), `clearHighlights`, `removeHighlightById` (surgical single-favorite unwrap), `rangeForHighlight`/`rangeForForkOrigin`, `repaintBody`/`repaintForkOrigins`/`repaintMergeLinks` (merge links paint as `<pythia-merge class="p-merge-link">`, ADR-130), `flashHighlight` |
...
└── PythiaSidebarView (sidebar.ts)
    ...
    ├── HighlightPainter       — favorite/fork/merge mark re-find + paint (findRange, repaintBody)
    ├── ForkController         — fork banner, origin marks, inline fork anchor
    ├── MergeController        — merge-link marks, inline anchor, merged-from banner (ADR-130)
    ├── longPress              — shared 450 ms press-and-hold gesture
    ├── dragToPan              — shared drag-to-scroll for wide content
    ├── tableDecorator         — wraps wide tables in a scroll frame (ADR-131)
...
persistData()  ← persist({ evict })
  ...
  → evict === true (saveConversations only): applyCap()
      → partitionEvictions(conversations, maxConversations, activeIds)
      → archiveBeforeEviction: NoteWriter.archiveConversationNote() per removed conversation
        ← a conversation whose note fails to write is KEPT, not deleted (ADR-172)
      → Notice: archived / removed / could-not-archive — never silent
