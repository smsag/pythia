...
| `services/MistralService.ts` | 310 | Mistral streaming (extends BaseProvider); uses `MistralCore` + tree-shakeable standalone `chatComplete`/`chatStream` functions, temperature/`reasoningEffort` (unrestricted per-model), resumeMode gating, bounded tool loop; PDFs unsupported this pass (warns via Notice) |
...
| `tests/` | — | Vitest unit tests (260 tests, ~1 s) |
...
- **Testing:** Vitest, 260 unit tests across 17 files, ~2 s. Coverage thresholds: statements/lines ≥ 90 %, branches ≥ 80 %, functions ≥ 95 %.
