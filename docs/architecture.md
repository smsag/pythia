...
| `services/ToolHandler.ts` | 300 | Tool definitions (`create_note`/`rewrite_note`/`prepend_note` + read-only `web_search` + read-only `upvoty_*`) + `ToolHandler` class (injected NoteWriter + optional WebSearchService + optional UpvotyService); `researchEnabled`/`upvotyEnabled` gate their tools independently of `writeMode`; `buildUpvotyArgs` maps tool input → MCP args |
...
| `services/UpvotyService.ts` | 300 | Read-only Upvoty feedback/roadmap access for "Upvoty mode": a minimal MCP-over-HTTP client (`initialize` + `tools/call` via Obsidian `requestUrl`, JSON/SSE parsing, stale-session re-handshake) targeting Upvoty's remote MCP server; `run()` never throws (returns an "Error:" string); pure `parseJsonRpcResponse`/`formatToolResult`/`readSessionId` are unit-tested |
