...
*Last updated: 2026-08-27 — structural-decomposition roadmap PR1 (ADR-103 / #120): extracted the quick switcher, history overlay, and delete-with-confirm into `ui/HistoryController.ts` (`Deps`-driven, follows `NavigatorController`); `sidebar.ts` 3,735 → 3,403 lines, ratchet ceiling lowered to match. `abbreviateModel` moved from `sidebar.ts` to `models/knownModels.ts` (next to `MODEL_ABBREVIATIONS`) so both the view and the controller share it. Behaviour-preserving; 434 tests still green.*
