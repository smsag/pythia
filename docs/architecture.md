...
*Last updated: 2026-09-18 — the embedding Worker on desktop (engineering-review #306). New `services/embedding/host/workerPrelude.ts` (`WORKER_PRELUDE`, `withWorkerPrelude`): both Worker paths — the blob Worker in `WorkerEmbeddingProvider` and the resource-path file from `embeddingWorkerUrl`, now `embedding-worker-<version>-p1.mjs` — run the bundle behind a prelude that hides Node's `process`, so transformers.js stops reading Electron's Node-enabled Worker as Node and accepts `wasm`. The iframe keeps the bare bundle. `FallbackEmbeddingProvider` gains `backend()` (`worker (blob)` · `worker (resource)` · `iframe (UI thread)`) and logs it once at info level. +11 tests (1305 across 87 files).*

*Previously: 2026-09-18 — the optimizer suggests a model (ADR-181). New pure `services/modelRecommendation.ts` (`DIFFICULTY_INSTRUCTION`, `parseDifficulty`, `requiredDepth`, `sendCost`, `recommendModel`) and `ui/ModelSuggestionController.ts` (the `.p-model-hint` chip: `consider` · `layer` · `sent` · `spent` · `clear`). `PromptOptimizerService.optimizeText(…, rateDifficulty)` returns `{ prompt, difficulty }`; `OptimizationController` gains `onRated`. `PythiaSettings.optimizerSuggestsModel` (default on). `sidebar.ts`: `turnConv = applyPendingTemplate(modelSuggestion.layer(conv))`, and the answer records and prices `turnConv.model` (was `conv.model` — wrong for a template's one-shot model). `ui/toolbarIcons.ts` takes the attach/save SVGs, keeping `sidebar.ts` at 1716. +24 tests (1294 across 86 files).*

*Previously: 2026-09-18 — request shaping follows the model (ADR-180). `models/knownModels.ts`: new `mistralReasoningEffort(model, level)` → `"none" | "high" | undefined`; `supportsEffort` now also marks `mistral-small-latest`/`mistral-medium-latest`, and `parameterSupport("mistral", …)` reads it. `MistralService` sends only the mapped value. `OpenAIProvider` loses `noSystemRole`: reasoning models get a system message, and `max_completion_tokens` keys off `isReasoningModel`. `update-models.mjs` suggests `supportsEffort` for adjustable Mistral reasoning. +5 tests (1270 across 84 files).*

*Previously: 2026-09-18 — first catalog curation (ADR-179 addendum). `MODEL_CATALOG` hides `gpt-4.1-nano`, `o3-mini`, `o4-mini` and adds `claude-fable-5-1`, `gpt-5.6`, `gpt-5.4-mini`, `gpt-5.4-nano`, `mistral-medium-latest` (profiles, guidance, prices). `scripts/update-models.mjs` gains `upstreamWindow`: `contextWindow` is `limit.input` where upstream gives one, else `limit.context`. `DEFAULT_SETTINGS.defaultOpenAIModel` is `gpt-5.4-mini`. +3 tests (1265 across 84 files).*

*Previously: 2026-09-18 — the model catalog is checked against models.dev (ADR-179). New `scripts/modelsDev.mjs` (`SOURCE_URL`, `UPSTREAM_PROVIDERS`, `UPSTREAM_IDS`, `NO_UPSTREAM`, `readCatalog`, `upstreamModels`, `upstreamId`, `nearbyIds`, `fetchUpstream` — shared with `update-pricing.mjs`, which re-exports them) and `scripts/update-models.mjs` (`readCatalogDetails`, `formatWindow`, `syncContextWindows`, `findNewModels`, `findDeprecated`, `suggestRow`, `renderReport`); `npm run update:models [-- --report <file>]`; `.github/workflows/update-models.yml` opens a PR for changed context windows and keeps one issue for new and deprecated models. `models/knownModels.ts`: five context windows updated from the first run. +16 tests (1262 across 84 files).*
