...
*Last updated: 2026-09-04 — embedding off-thread on capacitor:// builds (ADR-126): `blob:` Workers are blocked there, so the plugin writes the embedding bundle to `<pluginDir>/embedding-worker-<version>.mjs` once and starts the Worker from `adapter.getResourcePath(path)` (same-origin, blob-free). `WorkerEmbeddingProvider` takes an injected `spawnUrl`; the fallback chain is now blob Worker → resource-path Worker → iframe. Restores off-thread inference (no UI freeze) where blob is blocked; falls back safely to ADR-125's throttled UI-thread build otherwise.*
