...
*Updated: 2026-08-27 — model-picker guidance: users struggle to choose a model. Implementing the low-risk option — curated "good for" guidance + a per-provider Recommended tag in the model popover (see the in-flight work / ADR to follow). **#119 (backlog) — task-first model picker:** a top row of task chips in the model popover ("Quick question · Deep analysis · Long document · Coding · Creative"), each mapping to the right model for the active provider, so the choice is reframed from "which model" (jargon) to "what am I doing" (obvious). Deferred: needs a maintained task→model policy per provider and introduces a second selection paradigm next to the model list (power users may find it patronizing, and a wrong mapping actively misleads). Revisit if the audience skews non-technical or the "good for" labels prove insufficient.*
