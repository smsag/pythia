...
*Not fixed here:* the **conversations** and **scratch** folder pickers are in the scope too (they are what the index skips), and they live in the tab's own folder section, which has no handle on the row's refresh. Giving it one costs two lines in `settings.ts`, which is at its ratchet ceiling (528) — and the ratchet only ever shrinks. It belongs with the settings-tab split that ceiling is asking for. Much rarer than editing the indexed folders: it needs the user to move where conversations are stored *while* the settings tab is open.
