...
| 365 | **`WorkerEmbeddingProvider` and `IframeEmbeddingProvider` are the same postMessage client twice.** Both hold `Pending`, `request`, `onMessage`, the ping-proven ready poll and the two timeouts, differing only in how they mount the backend and in `isOffThread()`. #363 had to be fixed identically in both, which is the evidence for principle 4 ("one implementation per interaction"). | Medium | **Open.** Worth extracting the shared client, but not in a PR of behaviour fixes: the iframe path has no unit tests by design (it needs a real Obsidian window), so the extraction wants its own PR and a run on the device. |
