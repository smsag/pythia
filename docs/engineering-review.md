...
*Previously updated: 2026-09-25 — **#381: a card with buttons appeared half out of view.** A note-writing tool call (write_mode) puts a confirmation card into the conversation and the answer waits on it; at rest, its buttons sat below the fold. Cause: `runWrite` created the card EMPTY, set `scrollTop = scrollHeight`, and only then added the label and buttons — the scroll was measured against a card with no height. `runSearch` had the same order, and the cut-off (`.p-trunc`) and rewrite (`.p-rewrite`) cards were painted under a finished answer with no scroll at all. The direct `scrollTop` write also bypassed the view's `isScrolling` guard. Fix (ADR-215): one rule, `ChatScroll.reveal` in `ui/chatScroll.ts` — measure the card once built and scroll only as far as it takes to show the whole card; forced for the write confirmation (it blocks the answer), unforced for a status or a card under the answer (follows only a user who is following). The view's scroll state (`autoScroll`, `isScrolling`, the scroll listener, `scrollToBottom`'s body) moved into `ChatScroll`, which paid for the change: `sidebar.ts` 1505 → 1499. Verified in Chromium with real layout: old order leaves the card's bottom 44px under the view, new order shows it whole. `tests/chatScroll.test.ts` (10) fails on the old controller: it asserts the reveal happens forced and only after both buttons exist.*

*Previously updated: 2026-09-23 — **#379: the settings tab's information architecture (ADR-209).** ~42 controls under 11 headings, grouped by what a setting is made of rather than by what it answers. Two rows were genuinely mis-parented — `customInstructions` and "Debug mode" rendered after the embedding block, which creates its own headings, so both read on screen as vault-context settings — the glossary's three rows had no heading at all, and "Defaults" was a one-row section placed after the three sections it governed. Reorganised on one axis, **scope**: eight sections, each opening with a sentence naming its remit, and one section holding everything a conversation can override with that stated on every row. `settings.ts` 528 → 87 lines behind `ui/settings/`; #367's remainder closes with it. +16 tests.*

*Previously updated: 2026-09-22 — **#375: ADR-208's discussion section was deleted by the next re-lookup — the exact loss it was written to prevent.** `save` rebuilds the on-disk entry through `entryFromFrontmatter`, which never read the new field, so `mergeEntry` was always handed an `existing` with no discussion. The ADR's own tests passed throughout, because they built `existing` by hand instead of running the composition `save` performs — **a merge rule is only as good as the object it is merging into.** Found in review of the same PR, with #376–#378.*

*Previously: 2026-09-22 — **#372: a glossary definition is sometimes mute, and regenerate was the only repair.** Two symptoms, not one problem: "correct but says nothing" (which no prompt change fixes — what is missing is the questions the reader has not asked yet) and "the wrong sense" (which one sentence from the reader fixes). Now two exits sized to the two: a sense hint on the anchor, and a conversation forked from the term that distils back into its own section of the note (ADR-208).*

*Previously: 2026-09-22 — **#370: a multi-word term was held together by a literal space, one text node at a time.** German merges what English separates, so the English half of this glossary is nearly all phrases — and a phrase is what markdown breaks in half. `cartel-law`, a soft line break, `**Cartel** law` and any phrase overlapping a favorite all went unmarked. The matcher now tolerates the separator and the painter matches across the whole body (ADR-207). **The lesson is the correction itself:** this review had called the case rare one message earlier, from a mental image of English terms in English prose rather than from the vault the feature serves.*

*Previously: 2026-09-22 — **#368: a term defined in one language went unmarked in the other.** "Kartellrecht" was underlined, "cartel law" beside it was not. The index was never the problem: a translation is already a surface form like an alias — the entry just held none, because the lookup asked only for languages the passage itself used. It now asks for English and the conversation's language regardless, and the translation call backfills the entries written before it (ADR-206).*

*Previously: 2026-09-22 — **#367: adding a folder to index left the status row stale.** Reported on 2.25.1: the row still said "Bereit", *Jetzt aufbauen* stayed disabled, and only the destructive *Index neu aufbauen* worked. The index itself was right — a changed scope reads as out-of-date and the next build picks the folder up — but the row repaints only on index events, and a settings change is not one. Every scope control in the embedding section now refreshes it.*
...
## Bug (#367) — adding a folder left the index status stale, 2026-09-22

| # | Item | Severity | Status |
|---|---|---|---|
| 367 | **After adding a second folder to index, the settings row still said "Bereit" and *Jetzt aufbauen* stayed greyed out — the only working control was the destructive *Index neu aufbauen*.** Reported on 2.25.1. The engine was right throughout: a changed scope reads as `outdated`, and both an automatic build and *Build now* pick up the new folder's notes. The row was wrong. `renderVaultIndexStatus` disables *Build now* whenever the state is `ready` (correct — there is nothing to do), and it repaints on `onVaultIndexChange`, which fires on build progress. Changing a **setting** fires nothing, and the folders textarea, the note cap and the two skip-folder pickers are all part of `scopeSignature` — so the row kept the paint from before the change, and with it a disabled button, while the index it described was out of date. Only the model dropdown and the enable toggle had ever called the row's refresh. | Medium | **Fixed.** `renderEmbeddingSettings` returns the row's refresh; the folders textarea and the note cap call it, and `PythiaSettingTab` calls it after any folder pick (two of the four are in the scope; refreshing for all of them cannot go stale if a third joins). `tests/embeddingSettings.test.ts`: adding a folder enables *Build now* and the row says out-of-date; the cap does the same. Reverted to confirm the tests fail. |

*Why it looked like a regression:* nothing about this changed in 2.25.1 — the hole has been there since the status row shipped in 2.25.0 (ADR-199). It needs a scope change made while the tab is open, and a folder is the scope input people actually edit.

*Not fixed here, then:* the **conversations** and **scratch** folder pickers are in the scope too (they are what the index skips), and they lived in the tab's own folder section, which had no handle on the row's refresh. Giving it one cost two lines in `settings.ts`, which was at its ratchet ceiling (528) — and the ratchet only ever shrinks. It belonged with the settings-tab split that ceiling was asking for. Much rarer than editing the indexed folders: it needs the user to move where conversations are stored *while* the settings tab is open.

*Closed 2026-09-23 by #379 (ADR-209).* That split happened, and it carries the refresh as `SettingsContext.refreshIndexStatus`: `renderEmbeddingSettings` returns the row's refresh, the tab holds it, and both skip-folder pickers — now in "Notes Pythia writes" and "History and storage" — pass it to `folderRow`. A test reads each section's source and fails if a picker for `conversationsFolder` or `scratchFolder` does not pass it.

## Cross-language glossary marks (#368) — 2026-09-22

| # | Item | Severity | Status |
|---|---|---|---|
| 368 | **A term defined in one language went unmarked in the other.** Reported with a screenshot: an answer reading "Understanding Cartel Law (Kartellrecht)" marked the German word and left "cartel law" plain, in the same sentence. Nothing in the mechanism was broken — ADR-149 registers each `translations` entry as a surface form in the same alternation as an alias, and the painter has no rule that skips the phrase — the entry simply had no `term_en`. The lookup prompt asked only for "any OTHER language **the passage uses**" and to "leave the line empty if the passage is monolingual": English prose with one German word in parentheses reads as monolingual English with the term as the foreign body, and the far more common case (an all-German answer, the conversation switched to English later) reads that way too. A cross-language form was something that happened by accident. | Medium | **Fixed (ADR-206).** The lookup asks for English plus the conversation's own language regardless of the passage, with the old scope rule replaced by a quality bar — an established, domain-specific equivalent only, and nothing that would match unrelated sentences. `translateDefinition` carries `TERM:` beside `DEFINITION:` and records `term_<lang>` in the same frontmatter write that caches the translation, so entries written before this ADR fill in as they get read, with no migration and no extra request. `newTranslation` refuses a form equal to the term, one that repeats an alias, and any language the entry already answers for; `applyTranslation` never overwrites a value in the note and never treats a form as stale when the definition is reworded. |
| 369 | **`en: none` was a storable surface form.** The variants loop stripped bullets, quotes, annotations and placeholders; the translations loop, added later, had a shorter copy that checked only the length — so a model answering `TRANSLATIONS: en: none` stored "none" as a form and marked the word in every English answer. Principle 4, found while adding a third copy for ADR-206's translated term. | Low | **Fixed (ADR-206).** One `cleanSurfaceForm` in `services/glossaryReply.ts`, used by all three. It also strips a trailing `*`, which the leading class always did — a model writing `*Zählern*` left an asterisk welded to the form, and a form ending in one can never match. |

*Not done: a form split across inline formatting.* Matching is per text node, so `*cartel* law` does not match. German compounds make the multi-word English equivalent the normal case rather than an edge one, so this is worth its own decision — it is a change to the painter (flatten a block, match, map back, wrap per node), not to the glossary.

## Multi-word terms (#370) — 2026-09-22

| # | Item | Severity | Status |
|---|---|---|---|
| 370 | **A multi-word form was held together by a literal space, and matched one text node at a time.** Both are wrong for the English side of a German glossary, where a phrase is the normal shape of a term: German merges what English separates, so `Kartellrecht` → `cartel law`, `Preisabsprache` → `price fixing`. A literal space missed `cartel-law` and `cartel\nlaw` (a soft line break in the source lands in the text node as a newline); per-node matching missed `**Cartel** law` and any phrase overlapping the end of a favorite or fork mark. The German term, being one word, never splits — so every one of these failures landed on the half of the glossary ADR-206 had just filled. This review dismissed the formatting-split case as rare in the same session, on the strength of picturing English terms in English prose; the user's correction is what reframed it. | Medium | **Fixed (ADR-207).** The gap between two words matches any run of whitespace or a hyphen, and `repaintTerms` runs the alternation over the whole body text, painting a boundary-crossing hit as one mark per node — which `paintRange` has done for favorites since they shipped. Skipped regions (code, links, citation chips, another term mark) are masked to a same-length filler so a match can never straddle one. 3 mutations applied, 3 killed. |
| 371 | **A loosened matcher would have produced marks that do nothing when tapped.** Not shipped — caught while writing #370. `canonicalTerm` resolved a matched form through `normalizeTerm`, so the moment the alternation accepted `cartel-law` for the form `cartel law`, the matched text stopped being a key the canonical map held; the mark would carry a `data-term` no entry answers for, and tapping it would open nothing, with no error anywhere (principle 2). | Low | **Fixed with it (ADR-207).** `surfaceKey` folds exactly what the matcher varies and is the key on both sides — registration and resolution. A test asserts every spelling resolves to the one entry, and reverting the resolution fails it. |

## Mute definitions (#372) — 2026-09-22

| # | Item | Severity | Status |
|---|---|---|---|
| 372 | **A glossary definition is sometimes mute, and the only repair was regenerate.** Reported as two symptoms that are not one problem. *Correct but says nothing:* the definition restates the term at the level the reader already had — partly by design, since the prompt asks for two or three ISO-704 sentences on a 420-token budget, and no prompt change fixes it because what is missing is not in the model's first answer but in the questions the reader has not asked yet. *Defines the wrong sense:* the general meaning where the passage meant a specific one — cheap to fix, because the reader knows which sense they meant. Regenerate re-asks the same question and reliably gets the same answer. | Medium | **Fixed (ADR-208).** Two exits sized to the two problems. **"Other sense"**: a one-line field in the anchor whose text is appended to the lookup as a correction, with the model told never to silently define a third thing; not stored, like ADR-177's one-shot layer. **"Discuss"**: opens a conversation seeded with the term note (attached, not copied) and the passage, composer prefilled but unsent, `theme` pinned to the term; a header row then distils it into a `## Discussion` section of the note through a term-focused prompt that is deliberately not `generateSummary`. 5 mutations applied, 5 killed. |
| 373 | **The write-back could have destroyed the definition it was meant to complement.** Not shipped — caught while writing #372. `mergeEntry` protects a definition only when its source is `manual`, so saving a discussion as a bare `{ term, discussion }` would have let an empty incoming definition replace the real one on every model-written entry, which is most of them. | Low | **Avoided (ADR-208).** `saveDiscussion` hydrates the entry first, so the definition, contexts and every property go back through `save` exactly as they came off disk. A test passes an index entry with an empty definition and asserts the summarizer is given the hydrated one. |
| 374 | **`HeaderController` crossed ADR-097's 600-line ratchet holding a vault write.** The header owns a menu row, not a write to a glossary note — and it is the one file in the UI layer where a rule like this cannot be tested, because it needs a mounted view. | Low | **Fixed (ADR-208).** `ui/termDiscussion.ts` takes the rule behind a structural host interface (`TermDiscussionHost`), the shape ADR-205 used for the embedding hub; a test satisfies it with a plain object. `HeaderController` 604 → 585, no ceiling added. |
| 375 | **The discussion section was deleted by the next re-lookup — the exact loss ADR-208 was written to prevent.** `GlossaryService.save` rebuilds the on-disk entry with `entryFromFrontmatter(basename, fm, body)` and merges the fresh lookup into it. `entryFromFrontmatter` reads `definition` and `contexts` out of the body and **never read `discussion`**, so `existing.discussion` was always undefined at the merge and `incoming.discussion ?? existing.discussion` resolved to undefined. Pressing *Define again* on a term you had discussed silently rewrote the note without the section. **The ADR-208 test suite passed throughout**, because it exercised `mergeEntry` with an `existing` built by hand — which is not where `existing` comes from. | High | **Fixed in review.** `entryFromFrontmatter` carries `body?.discussion`, and its doc names the rule: a caller that has the body must pass all of it, because `save` merges into what this returns, so a field missing here is a field the next write deletes. Two tests now run the real composition — render → parse → `entryFromFrontmatter` → `mergeEntry` — rather than `mergeEntry` alone; removing the line fails both. |
| 376 | **The discussion summary had a smaller token budget than the shorter summary it is modelled on.** 700 against `generateSummary`'s 1024, for a contract of 8 sentences against its 5, on the conversation's model — which may be a reasoning model, where the same budget also pays for hidden reasoning. ADR-141 says this in so many words: lowering a cap truncates rather than shortens. | Medium | **Fixed in review.** 1024, with the reason in the code. A test asserts the budget is not below the summary's. |
| 377 | **A discussion too long to fit was truncated from the wrong end.** `conversationText.slice(0, 24000)` keeps the opening of a discussion and drops the conclusions — and understanding is where a discussion lands, not where it starts. `generateSummary` does not slice at all, so this was a new truncation, pointed backwards. | Medium | **Fixed in review.** `slice(-DISCUSSION_CHARS)`. A 120-turn conversation is asserted to reach the model with its last turn and without its first. |
| 378 | **A failed fork reported a failed definition.** `discuss()` caught its error into `glossaryLookupFailed` — "Could not define the term" — which is not what failed. Not silent, but misleading, which is the same bug wearing a better coat. | Low | **Fixed in review.** `glossaryDiscussFailed`, naming the term and the real failure. |
| 380 | **The discussion was written and never shown in Pythia.** Raised in the review of #372: the section was only visible by opening the note, so the next time the reader met the term they got the same gloss that was mute the first time — the compounding the feature exists for never reached the surface where terms are met. | Medium | **Fixed.** Rendered in the term anchor under the definition and in full (no fold — ADR-189), sharing the anchor's existing label and body rules. Not translated, with the label naming its language when that differs from the anchor's. 2 mutations applied, 2 killed. |

## Refactor (#379) — the settings tab's information architecture, 2026-09-23

| # | Item | Severity | Status |
|---|---|---|---|
| 379 | **The settings tab grouped by what a setting is made of, not by what it answers — and two rows had fallen out of any section at all.** ~42 controls under 11 headings in one scroll. (a) `customInstructions` and "Debug mode" were emitted *after* `renderEmbeddingSettings`, which creates its own two headings, so both rendered under "Vault context (semantic RAG)" and read as vault-context settings — a real defect, not a preference: a section was a bare `createEl("h3")` with no scope, so nothing in the code could say where one ended. (b) The glossary's three rows had no heading, rendered inside "Vault folders", so they read as folder settings. (c) "Defaults" was a one-row section (`defaultProvider`) placed *after* the three provider sections it decided between. (d) "Behaviour" and "Features" were dumping grounds — ten unrelated rows under the first. (e) Two heading mechanisms, `createEl("h3")` and `setHeading()`, which do not render alike. Underneath all five: "Vault folders" grouped by value *type*, which is exactly why `archiveFolder` had to be exiled into "Behaviour" beside the toggle that uses it. And the distinction the code cares most about — principle 6's overridable-vs-global — was stated nowhere in the UI. | Medium | **Resolved (ADR-209).** One axis: **scope**. Eight sections — Connections · New conversations · While answering · Prompt optimizer · On-device semantic search (+ Vault context) · Notes Pythia writes (+ Glossary) · History and storage · Troubleshooting — each opening with one sentence naming its remit (extended from the embedding block, which was the only part of the tab that did this). **Only "New conversations" is overridable, and every row there says so** through one shared sentence; that is what moved `customInstructions` to the global section, where it belongs because it applies to conversations already underway. **A folder lives with the feature that writes to it**, so "Vault folders" is gone and the archive folder sits under its toggle. **One model row** for the chosen provider, replacing three headings with two rows each. `settings.ts` 528 → **87** lines, a shell over `ui/settings/` (one module per section over one `SettingsContext`); its ratchet entry is deleted. Locale strings split into `locales/settings.{en,de}.ts` — both files sat just under the 600-line default and the intros would have broken it. +16 tests in `tests/settingsIA.test.ts`, five of the rules broken at once to confirm five failures. No setting's meaning, stored key or default changed. |
...
| **A selection in a tab was saved against the kept answer** — invisible star, misplaced merge link or pin, Branch from the wrong answer. Found in the review after merge. | High | Closed (ADR-225): the tab view carries its own `data-msg-id`; Branch hidden and refused on a tab |
| **Delete / Retry left favorites and merge links on the tab ids behind**; a tap on one in "Starred" did nothing and said nothing. | High | Closed (ADR-225): `answerIds` in `spliceExchange`; a favorite found nowhere shows a Notice |
| **Retry dropped the tabs** — its guard counted only marks on the kept answer. | Medium | Closed (ADR-225): withheld on an answer with tabs |
| **"Use this answer" lost `truncated` and `rewriteTarget`.** | Medium | Closed (ADR-225): `ComparisonCandidate` carries both, validated on load |
| **A jump to a mark on a tab not on screen found nothing.** | Medium | Closed (ADR-225): `findAnswerEl` brings the tab up first |
