...
*Updated: 2026-09-15 — the bottom strip, sixth report, and the instrument that ends the guessing (ADR-148). Ask: remove a colour ramp visible below the composer on mobile, and move the button row lower so the transcript gets more room. Measured off the reported screenshot (DPR 3): the ramp is **linear** over ~36 CSS px and **inset ~11px on both sides while the panel is flush**, so its painter is an ancestor in Obsidian's mobile drawer — `.p-input-area` is the last flow child of `.pythia-view`, and every `linear-gradient` in `styles.css` is accounted for (two clamp masks, two highlighter fills, one navigator mask). What the screenshot cannot settle is whether the band sits inside our panel or below it: **both surfaces are `--background-primary`**, and the same inference underpins ADR-146's "42px". Rather than guess a sixth time, shipped `ui/layoutReport.ts` + a `Diagnose panel layout` command (ancestor rects/padding/margin, their `background-image`/`box-shadow`/`mask-image`, and a hit test from the composer's bottom down to the viewport's, written to a note because the bug only appears on a phone). Shipped alongside it, because it is unambiguously ours: `.p-input-area` padding-top `--s2`→`--s1` and gap `--s3`→`--s2`, 8px of transcript back. Open until the report lands: whether the remaining ~36px is reclaimable at all. +10 tests (775 across 52 files); lint, check:filesize, build and tests green.*
