...
| # | Item | Severity | Status |
|---|---|---|---|
| 303 | **Mistral is sent effort levels its models do not list.** `MistralService` sends `reasoningEffort` whenever one is set, on any model (the SDK types allow any value). models.dev lists `none` · `high` for both `mistral-small-latest` and the new `mistral-medium-latest`, and Pythia's `EffortLevel` is `low` · `medium` · `high`. Nobody has reported a 400, so either the API tolerates the extra values or nobody has combined the two. Verify against the live API; if it rejects them, map `low`/`medium` to the nearest accepted value per model, and `parameterSupport` should stop claiming `effort: true` for every Mistral model. | Medium | Open |
| 304 | **GPT-5 gets its system prompt as a user turn.** `OpenAIProvider` sets `noSystemRole = isReasoningModel(model)`, a rule written for o1, which had no system role. GPT-5 (and today's o-series) accept `system`/`developer`. Folding the prompt into a `[System instructions]` user message works, but gives the instructions less weight than the role would. Split the flag from `isReasoning` once there is a reason to trust the role on every catalog reasoning model. | Low | Open |
