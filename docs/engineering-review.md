...
| **A phone on a desktop-kept index could neither see that nor end it**: nothing said so, Build now was greyed out when ready, and on a UI-thread backend it returned without syncing. | Medium | Closed (ADR-220 addendum): the status row says so with the date; Build now takes the index over |
