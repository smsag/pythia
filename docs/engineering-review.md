...
*Updated: 2026-08-27 — structural decomposition roadmap (#119–#122) added, and PR0 landed. Comparing Pythia against the obsidian-similarity plugin surfaced one real gap: the `services/` layer is cleanly factored, but `sidebar.ts` (3,643 lines, ~105 methods) and `main.ts` (951 lines) are god-objects with nothing preventing regrowth. PR0 establishes the guardrail — a file-size ratchet (`scripts/check-file-size.mjs`, 600-line default + grandfathered monolith ceilings, wired into CI ahead of the build) — and the first tested seam of the riskiest target: `services/sendPolicy.ts` lifts `sendMessage`'s two pure post-turn trigger predicates behind `tests/sendPolicy.test.ts` (8 cases). #119–#122 (the controller-extraction sequence, the `main.ts` split, the `AppContainer` + `ConversationStore` ownership inversion, and the lock-in) remain open. See ADR-097.*
...
## New Suggestions (#119–#122) — structural decomposition roadmap, 2026-08-27

Prompted by a side-by-side comparison with the obsidian-similarity plugin. Pythia leads on process (PR CI, breadth of tests, docs/ADRs, the `BaseProvider`/`LLMRouter` provider abstraction) but trails on one dimension: **uniform structural discipline**. `sidebar.ts` (3,643 lines, ~105 methods) and `main.ts` (951 lines) are god-objects, and nothing stopped them re-growing. This roadmap closes that gap the way the repo already closed it in the `services/` layer and in `NavigatorController`/`OptimizationController`: extract cohesive clusters into `Deps`-driven controllers, one behaviour-preserving PR at a time, with a CI guardrail so the monoliths can only shrink. Full rationale and the rejected alternatives are in ADR-097.

### #119 — Decompose `sidebar.ts` into `Deps`-driven controllers

**Files:** `sidebar.ts`, new `ui/*Controller.ts` — **Open (PR0 seam landed)**

`PythiaSidebarView` mixes header/model/rename chrome, summary cards + context inspector, fork anchors/menus, selection + favorites, message rendering + delete-last-exchange, the input composer, quick-switcher/history, and the 270-line `sendMessage` send/stream orchestration. Extract in risk-ascending order into controllers following the existing `NavigatorController` pattern (a `Deps` interface carrying the plugin, the specific DOM elements, and callbacks into the view), leaving thin delegating facades for methods `main.ts` calls (`attachNoteToInput`, `prefillInput`, `triggerAutoPrompt`, `scrollToMessage`, `summarizeFavorites`, `handleDeleteConversation`, `onCopyConversationLink`, `getActiveConversation`) so `main.ts` is untouched per PR. Planned clusters: HistoryController → SummaryController → ContextInspector → ForkController → SelectionController → HeaderController → TranscriptRenderer → Composer/SendController (last, highest-risk). After this, the view is a thin coordinator (lifecycle + `setActiveConversation` + wiring). **PR0** lifted the first tested seam of the `sendMessage` target: `services/sendPolicy.ts` (`shouldGenerateTitle`, `shouldGenerateChapterName`) with `tests/sendPolicy.test.ts`.

### #120 — Decompose `main.ts` (god-plugin) into focused services

**Files:** `main.ts`, new `services/*` — **Open**

`onload` and the plugin class own loading, secrets, persistence, leaf/view management, and eight `cmd*` handlers. Split into: `SecretStore` (`setApiKey`/`setOpenAIKey`/`setMistralKey`/`setSearchKey` + the async `getSecret`); `PluginDataStore` (`loadPluginData`/`saveSettings`/`saveConversations`/`persistData`/`watchDataJson`/`reloadFromDisk` — the I/O shell around the already-pure `services/persistence.ts`); `ConversationService` (`createConversation*`, `renameConversationFile`, the `cmd*` handlers, which register in `onload` but delegate here); and `ViewManager` (`initLeaf`/`activateView`/`getSidebarView`). The plugin shrinks to registration + delegation.

### #121 — `AppContainer` composition root; invert `ConversationStore` ownership

**Files:** new `AppContainer.ts`, `main.ts`, `services/ConversationStore.ts` — **Open**

Replace imperative `onload` wiring with one composition root (mirroring obsidian-similarity's `appContainer.ts`) that constructs the services and exposes them as readonly fields; `onload` becomes `new AppContainer(this)` + register view/commands/events. Then invert the one coupling the hexagonal comparison flagged: `ConversationStore` currently mutates `plugin.conversations` (bidirectional). After `PluginDataStore` exists, make the store the **owner** of the array and turn `plugin.conversations` into a read accessor — persistence flows through the store, not back through the plugin. Extends #11 (partial sidebar split) and #94 (`buildUI` decomposition).

### #122 — File-size ratchet guard + per-controller unit tests

**Files:** `scripts/check-file-size.mjs`, `.github/workflows/ci.yml`, `package.json`, `tests/*` — **Resolved (guard); ongoing (tests)**

**Resolution (guard):** added `scripts/check-file-size.mjs` — a 600-line default budget for every `.ts`, with explicit grandfathered ceilings for `sidebar.ts` (3,643) and `main.ts` (951) that act as a ratchet (each #119/#120 extraction must lower the matching number, never raise it). Wired into CI as a `Check file-size budget` step ahead of the build, and exposed as `npm run check:filesize`. A new file over the default, or a grandfathered file grown past its ceiling, fails CI. **Ongoing:** each extracted controller (#119) becomes unit-testable in isolation for the first time — capture that as its cluster lands, bringing the UI surface toward the coverage the `services/` layer already has.
