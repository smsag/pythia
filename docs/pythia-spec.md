...
| D-3 | **The model can change the vault and nothing says so.** `writeMode` defaults to `all`; no surface shows it, and a conversation cannot be made read-only. | #255 | Open, High. Needs a product call on the default for a template-less conversation, then an ADR. | Next conversation-controls pass. |
| D-4 | **Resume mode silently drops history.** `summary` sends no prior messages, `hybrid` only the last six, for every later send. Nothing says so and nothing switches back. | #256 | Open, High. | " |
| D-5 | **Web-search state is ambiguous** — on · off · auto-armed · no-key are four states shown as two. | #257 | Open, High. | " |
| D-6 | **The system prompt is a black box** — a template silently resets six fields and the text cannot be read from the conversation. | #258 | Open, Medium. | " |
...
| # | Decision | Review | Position today | Revisit when |
|---|---|---|---|---|
| D-13 | **Vault-RAG retrieval floors are unmeasured.** ADR-169 measured conversation pairs; `vaultRetrievalMinScore` (0.5 / 0.35 / 0.2) is on faith. | #273 | The two maps are deliberately separate and a test fails if they are merged. | Before touching either constant. Needs a query-to-note probe like `measure-related.mjs`. |
...
| D-15 | **Provider SDKs have no runtime test.** Three majors were bumped; `tsc` and the bundle passed, no real call was made. | #290 | A release gate, not a merge gate. | **Before the next version tag:** one streamed chat and one utility call per provider in a real vault. |
