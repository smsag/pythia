...
| 2026-09-18 | D-28…D-32 added from ADR-179 (embedding off the UI thread, a resumable vault build): the live-verification the fix still needs, backend recycling, the iframe fallback's future, why `truncation` was not taken, and the append-only index that would remove the flush-rate trade-off. |
