...
| D-59 | **A phone that embeds no vault notes ("read-mostly").** **Closed by ADR-220 (2026-09-26), in a different shape:** a phone still embeds its own edits, in memory, so its answers see them; it no longer writes an index a desktop keeps, and a desktop catches up once per launch on what changed while it was closed. Embedding nothing would have left a phone blind to what was just written on it. | ADR-219, ADR-220 | The residue: a phone's in-memory edits are gone at its next launch until the desktop has caught up. | A phone answering from stale vectors where the desktop rarely runs. |
...
| D-35 | **An append-only vault index.** **Closed by ADR-221 (2026-09-26):** not appended but journaled — an edit rewrites a small journal of the rows changed since the index was written, and the index itself is rewritten only by a build, a takeover, or when the journal reaches 5 % of it. | #310, ADR-221 | Appending proper needs `appendBinary` (Obsidian 1.12.3); the journal is small enough that rewriting it costs nothing. | The status line's count before the index is loaded leaves out the journal (ADR-221's known gap), if that ever misleads. |
...
| Date | Change |
|---|---|
| 2026-09-26 | ADR-221: D-35 closed — the vault index keeps a journal of its edits. |
| 2026-09-26 | ADR-220: D-59 closed — a phone holds a desktop's index in memory and no longer writes it; a desktop catches up once per launch. |
