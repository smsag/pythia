...
| Settings: index status | a `Setting` row whose description is `.p-index-status-headline` + `.p-index-status-detail`; buttons *Build now* · *Rebuild index* | `ui/vaultIndexStatusSetting.ts` | the plugin settings tab, Vault context section (ADR-198) |
...
| D-39 | **The related-conversations index is not behind the build guard.** ADR-198's crash-loop breaker wraps the vault index only. | ADR-198 | That index is built only when the user asks for related conversations, and the background warm is desktop-only (ADR-169), so it cannot loop by itself; since ADR-198 it also uses the device's model, so a phone no longer loads the multilingual one for it. | A crash report from the related panel on a phone, or the warm is ever enabled on mobile. |
| D-40 | **A multilingual model a phone can hold.** ADR-198 gives phones the English model, losing cross-language matching there. | ADR-198 | The multilingual MiniLM added ~0.9–1 GB to the iOS web process; its 250 k-token vocabulary is the likely cost, which a smaller multilingual model sharing that vocabulary would not remove. Any candidate is measured on the phone first (`ios_webkit_debug_proxy` + `idevicesyslog`, ballast to find the baseline) and marked `mobile: true` only on the numbers. | A user whose phone questions and notes are in different languages, or D-36's retrieval-model spike picks a candidate. |
...
| Date | Change |
|---|---|
| 2026-09-22 | ADR-198: the settings index-status row added to the UI map; D-39 (the related index outside the build guard) and D-40 (a multilingual model a phone can hold). |
