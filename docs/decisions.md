...
*Previously: 2026-09-26 — ADR-219 (the note being written is held back from the vault index until it is left or quiet for 30 s; the watcher's flush is a real quiet window; edit batches share one write window, so typing no longer rewrites the ~19 MB index every two seconds — the cause of Obsidian reloading on the iPhone while editing, confirmed by switching vault context off).*
...
### ADR-220 — A desktop keeps the vault index; a phone holds it

**Status:** Active · 2026-09-26

**Context.** ADR-219 took the note being written out of the index. Two gaps were left behind it, both found while scoping D-59.
...
**Why not the D-59 shape (a phone that embeds nothing).** It would have made a phone's answers blind to what was just written on it until the desktop ran. Embedding in memory costs one note on leaving it (ADR-219). The whole-file write was the expensive half, and that is what goes.
