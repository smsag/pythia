...
*Last updated: 2026-07-17 — ADR-045 (Mistral added as a third LLM provider — streaming, tool-calling, temperature/effort/maxTokens parity — plus the two-way-provider-ternary audit and fix, and two bonus shared-code fixes to `retry.ts`/`apiError.ts` surfaced during implementation).*
