...
*Last updated: 2026-08-27 — ADR-099 (web search auto-arms for a single send when the outgoing message reads as time-sensitive and the research toggle is off — a per-turn armed clone offers `web_search` without persisting `researchMode`; the globe pulses to show it fired; trigger wording in the tool description and recency nudge strengthened to a search-first default; `webSearchAutoArm` setting, default on).*
