...
*Last updated: 2026-09-16 — ADR-163 (the cost of an answer is an estimate from a date-stamped table, shown where the tokens already are; addenda: the table is a user-editable setting behind a disclaimer that says estimate, not bill; the cost is snapshotted on the message at generation; prices update through a weekly models.dev pull request, never a fetch; off by default). `≈ $0.012` follows the token counts on every assistant turn label, computed at render time from `models/modelPricing.ts`; a running total per conversation sits in the history panel; the next-send token estimate beside Send is removed. Off-switch in settings.*
