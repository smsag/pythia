...
**Guards.** `tests/buttonRoles.test.ts` (every button names a role; each role keeps its label and fill at rest and hovered) and `tests/obsidianCascade.test.ts` (no sentinel from Obsidian's ten measured rules reaches a role). Both flatten the contract's `var()` fallbacks, which happy-dom does not resolve. Both test this repository against its host, and name no other repository.
