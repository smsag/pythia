...
**Consequence:** one obvious way to find a conversation — a labelled loupe that opens a focused, keyboard-drivable search/browse panel. Fewer surfaces, one code path, less CSS. **Watch-item:** the panel's DOM (loupe→open→focus, ↑/↓/Enter, browse-vs-search branch) still has no automated coverage — the integration-gap class ADR-105 flags; add a `viewRender`-style smoke test when touching it again.
