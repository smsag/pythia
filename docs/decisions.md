...
*Last updated: 2026-09-15 — ADR-148 (two surfaces, one colour: the strip is measured, not guessed). Sixth report of the space below the composer, the first showing what is *in* it: a **linear** ramp `rgb(27)`→`rgb(33)` over ~36 CSS px, inset ~11px on both sides while the panel itself is flush — so the painter is an ancestor in Obsidian's mobile drawer, not `.pythia-view` and not our leaf container. Pythia paints nothing there: `.p-input-area` is the last flow child of the panel, and every `linear-gradient` in `styles.css` is accounted for. What the screenshot **cannot** settle is whether the band is inside our panel or below it — **both surfaces are `--background-primary`**, so the pixels are identical either way, and ADR-146's "42px" rests on the same inference. Shipped the instrument instead of a sixth guess: `ui/layoutReport.ts` + a `Diagnose panel layout` command reporting every ancestor's rect/padding/margin, their `background-image`/`box-shadow`/`mask-image`, and a hit test down to the viewport. Also reclaimed what is unambiguously ours — `.p-input-area` padding-top `--s2`→`--s1` and gap `--s3`→`--s2`, 8px of transcript. **Two surfaces that share a colour cannot be told apart in a screenshot; a boundary you can only infer is not a measurement.** +10 tests (775).*
...
### ADR-148 — Two surfaces, one colour: the strip is measured, not guessed

**Status:** Active — supersedes the *method* of ADR-132, 134, 135, 146 and 147; changes none of their code

**Context:** the sixth report of the space below the composer, and the first to come with a picture of what is *in* it rather than only of where it is. The report names two things: a "nicht durchgängiger Farbverlauf" — a colour ramp that does not carry through — and a wish for the button row to sit lower so the transcript gets more room.

**What the screenshot actually contains.** Measured off the reported image (1188×2576, DPR 3, dark theme, right-hand mobile drawer):

| device rows | what is there |
|---|---|
| … – 2135 | the accent Send button, ending at CSS y 711.7 |
| 2140 – 2255 | a ramp from `rgb(27)` to `rgb(33)` in **equal ~16px steps**, inset 34 device px on the left and 32 on the right |
| 2140 – 2255, outside that inset | `rgb(26)` — `--background-primary`, unchanged |
| 2256 – | `rgb(34)` = `--background-secondary`, full width, then the drawer's tab rows (`Pythia`, `Hochfahren`) at a ~45 CSS px pitch |

Three things follow. The ramp is **linear**, so it is a `linear-gradient` and not the tail of a box-shadow, whose falloff is gaussian. It is **inset ~11 CSS px on both sides** while the panel itself is flush to the leaf on both sides (`rgb(26)` reaches device column 1186), so ADR-147's rule is in effect and the painter is neither `.pythia-view` nor `.workspace-leaf-content` — it is an ancestor with its own horizontal padding. And the band is **~36 CSS px tall**, sitting between the composer and the drawer's tab list.

**Pythia does not paint it.** `.p-input-area` is the last flow child of `.pythia-view` — `buildUI` appends the header, the context bar, the chat wrapper, the reference row and the input area, in that order, and nothing after it. Every `linear-gradient` in `styles.css` is accounted for: two clamp masks (`.p-clamped`, `.p-bubble-collapsed`), the two highlighter fills, and the navigator title's right-edge mask. None of them can render below the composer.

**What could not be settled from the screenshot, and why.** Whether the 36px band lies *inside* our panel (dead space we own) or *below* it (drawer chrome we do not) turns on where the panel's bottom edge is — and **both surfaces are `--background-primary`**. The pixels are identical either way. Reading the band as ours gives "40px below the send button where 4 was intended", which is almost exactly ADR-146's 42; reading it as the drawer's gives "4px, exactly as designed, and five ADRs have been chasing someone else's chrome". The image supports both, and a sixth guess between them is worth nothing.

That cuts back through the record: ADR-146's headline measurement — "the panel's background runs to 2261 before the surface below it begins" — is the same inference, and rests on the same assumption that the first colour change below the composer marks the panel's edge. It may; it is not measured.

**Decision:** ship the instrument instead of the sixth guess.

New `ui/layoutReport.ts` and a `Pythia: Diagnose panel layout` command write a report into the scratch folder and open it. It reports what only the device holds:

- the rect, padding, margin and overflow of **every ancestor** from `.p-input-area` outwards to `<body>` — which names the box that creates any space below the panel, and settles the question above in one line;
- the **`background-image`, `box-shadow` and `mask-image`** of each of them — which names the gradient's owner as a selector;
- a **hit test** (`elementsFromPoint`) at five heights from the composer's bottom to the viewport's — which names whatever is painted in the strip even if it is a pseudo-element on a class this codebase has never heard of.

It probes down to the *viewport*, not to the panel's own bottom edge: a probe that stopped at our edge could never answer whether the strip is ours.

**Why a note and not a console log or the clipboard.** This strip has only ever been seen on a phone. There is no console to read there, and a long clipboard is awkward to hand back. A note syncs, opens, and can be quoted from at leisure; it is rewritten in place, so repeated runs do not litter the vault.

**What also shipped, because it is unambiguously ours.** `.p-input-area` goes from `padding: var(--s2) var(--s3) var(--s1)` to `var(--s1) var(--s3) var(--s1)` and its column gap from `--s3` to `--s2`: 8 CSS px of transcript back, with the composer reading as one control rather than two stacked rows. The 8px above the textarea was inherited from a desktop-sized composer and the `border-top` already separates it from the transcript. This is a real but small answer to "more reading room"; the larger answer is the 36px, and that waits for the report.

**Alternatives rejected.** *A `:has()` rule against the drawer's tab container* — three guesses stacked (the class name, the nesting, and which property creates the band), and if the gradient is an overlay rather than a gap, growing the panel into it would put the composer under a fade. *Runtime measurement driving a style* — ADR-146 retired exactly that pattern; this measures to **report**, never to style. *Asking the reporter to try CSS snippets* — the report answers in one run what a snippet answers in one round each.

**The rule worth keeping:** **two surfaces that share a colour cannot be told apart in a screenshot, so a boundary you can only infer is not a measurement.** ADR-147 said to go and get different evidence when a fix fails twice; this says what to do when the different evidence is still a picture. Build the thing that reads the layout.

**Consequence:** +10 tests (775 across 52 files) covering the report's pure half — the selector-shaped labels, the empty-strip and empty-painter cases, and the two gaps being stated separately. The reading half runs only on a device, which is the point. Build, lint, file-size and tests green. Not runtime-verified in Obsidian.
