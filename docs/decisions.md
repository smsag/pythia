...
*Last updated: 2026-09-23 — ADR-209 (the settings tab is organised by scope: one section holds what a conversation can override and says so on every row, a folder lives with the feature that writes to it, and `settings.ts` becomes a shell over one module per section).*
