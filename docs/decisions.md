...
*Last updated: 2026-09-16 — ADR-162 (the plugin's own icon: one registered mark for every entry point, drawn to Lucide's rules so it reads as native; `bot` retired).*
...
### ADR-162 — One registered icon for every entry point, drawn to Lucide's rules
