...
//
//   1. FACTS it rewrites: each catalog model's `contextWindow`. Upstream is the
...
export function syncContextWindows(source, upstream, catalog, ids = UPSTREAM_IDS, noUpstream = NO_UPSTREAM) {
	...
	for (const { id, provider, contextWindow } of catalog) {
		...
		const next = formatWindow(row.limit.context);
		...
		changes.push({ id, from: contextWindow, to: row.limit.context });
	}
	if (missing.length) {
...
export function suggestRow({ id, provider, row }) {
	let window;
	try { window = formatWindow(row.limit?.context); } catch { window = "/* unknown */ 128_000"; }
...
export function renderReport({ newModels, deprecated, asOf }) {
	...
	if (newModels.length) {
		out.push("", `## New upstream models (${newModels.length})`, "", "Chat models (text in and out, tool calls) released on or after the newest model the catalog carries for their provider. Status and flags are models.dev's.", "", "| Provider | Model | Released | Context | Note |", "|---|---|---|---|---|");
		for (const m of newModels) {
			const note = [m.row.status === "beta" ? "beta" : "", m.row.experimental === true ? "experimental" : ""].filter(Boolean).join(", ");
			let ctx; try { ctx = formatWindow(m.row.limit?.context); } catch { ctx = "?"; }
