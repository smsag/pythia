...
/
  ...
  ui/
    ...
    AnswerTabsController.ts   ← the tabs a kept comparison leaves on its answer: the kept tab first, another shown in place, "Use this answer" on the last answer only (ADR-219); findAnswerEl — the one way to reach an answer by id (ADR-225)
...
- **Auto-search arms on a time-sensitive question OR a link** (`wantsWeb` = `looksTimeSensitive` || `containsWebUrl`, ADR-229 — the user's decision; ADR-226's link-only reading was wrong). A date or cue word inside a `[[note link]]` does not count; `www.…` and `host/path` links do, a bare domain in prose does not
