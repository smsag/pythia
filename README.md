...
- **Related conversations** — the ⇄ icon on any row in the conversation panel (or a long-press on touch) finds conversations that are semantically close to it. With Schreibstube's *Search by meaning* on, Schreibstube answers with its model, so only one model runs on the device; otherwise Pythia's own on-device model does. Nothing is sent anywhere, and the similarity threshold is measured per model rather than guessed — *strict* / *balanced* / *loose* in the settings
