...
- **Related conversations** — the ⇄ icon on any row in the conversation panel (or a long-press on touch) finds conversations that are semantically close to it. Schreibstube's *Search by meaning* answers, with its model, on your device; without Schreibstube, the list stays empty. Schreibstube's **Recommended** panel also lists the conversations that belong with an open note; pressing one opens it here. Nothing is sent anywhere
