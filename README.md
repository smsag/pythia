...
- **`#` note picker** — type `#` in the chat input to fuzzy-search all vault notes and attach one inline, just like VS Code's `#` file picker. The note's name stays where you put it as an Obsidian `[[link]]`, so you can see what you attached while you write; delete the link and the note detaches again. It goes out with the message and renders there as a link you can open
