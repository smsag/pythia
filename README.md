...
- **Cost per answer** — every answer's label shows the token counts and an estimated price (`↑1.628 ↓941 · ≈ $0.012`), priced from a date-stamped list-price table you can correct under *List prices* in settings; the conversation panel shows a running total per conversation. An estimate, not a bill — your provider's invoice is authoritative. Switch it off under *Show cost per answer*
