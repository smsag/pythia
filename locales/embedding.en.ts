...
const embeddingEn = {
	...
	embeddingModelDesktopNote: "Phones and tablets always use {{model}} — {{chosen}} needs more memory than they allow.",
	...
	vaultIndexStateOutdated:     "Out of date — the folders or the note limit changed since it was built. It rebuilds the next time it's needed, or press Build now.",
	vaultIndexStateFailed:       "The last build failed: {{error}}",
	vaultIndexStateOutOfMemory:  "Stopped — this device ran out of memory while loading the model. Pythia won't retry on its own; press Build now to try again.",
	vaultIndexStatePaused:       "Paused — the last {{count}} builds ended without finishing. That happens when the app is closed mid-build or the device runs out of memory, so Pythia won't start another on its own. Press Build now to try once more.",
